...
export function TestCasesPage({ projectVersion, onManageVersions, notify }: { projectVersion: ProjectVersion | null; onManageVersions: () => void; notify: Notify }) {
  ...
  return <section className="test-cases-page" aria-label="测试用例">
    ...
    {!loading && !error && data?.libraryVersion && <>
      ...
      <div className={`test-cases-layout${selected ? ' has-detail' : ''}`}><section className="test-cases-table"><header><b>当前筛选结果</b><span>{items.length} / {data.statistics.total} 条</span></header>{items.length ? <div role="table"><div className="test-cases-row labels" role="row"><span>用例名称</span><span>来源</span><span>优先级</span><span>测试维度</span><span>执行方式</span><span>执行就绪状态</span><span>Revision</span></div>{items.map(item => <button className={`test-cases-row ${selected?.caseId === item.caseId && selected.revision === item.revision ? 'selected' : ''}`} key={`${item.caseId}:${item.revision}`} onClick={() => setSelected(item)}><span><b>{item.content.title}</b><small>{item.content.requirementRefs.join(' · ') || '未标注需求'}</small></span><span><Tag tone={item.source}>{sourceLabel[item.source]}</Tag></span><span>{item.content.priority}</span><span>{dimensionLabel[item.content.dimension] ?? item.content.dimension}</span><span>{item.content.executionMethods.map(method => method.toUpperCase()).join(' / ')}</span><span><Tag tone={item.executionReadiness}>{readinessLabel[item.executionReadiness]}</Tag></span><span>r{item.revision}</span></button>)}</div> : <div className="test-cases-no-results">没有符合筛选条件的正式用例。</div>}</section>{selected && <CaseDetail item={selected} onClose={() => setSelected(null)} />}</div>
    </>}
  </section>
}
...
function CaseDetail({ item, onClose }: { item: PublishedTestCase; onClose: () => void }) {
