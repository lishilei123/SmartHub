  ...
  return <section className="tdw-panel tdw-publications">
    ...
    {!versions.length ? <div className="tdw-empty"><PackageCheck /><b>尚未发布正式用例库版本</b><span>从设计任务完成 Proposal 处置与 Coverage Audit 后发布。</span></div> : <div className="tdw-master-detail">
      ...
      {selected && <article className="tdw-release-detail">
        ...
        <section ref={handoffRef} className="tdw-handoff-box"><h4>创建执行交接</h4><p>冻结测试用例 Revision、执行配置、追溯、内容 Hash 和独立测试数据需求快照；实际 Fixture / 生成器 / 数据引用到创建执行 Run 时再绑定。</p><p className="td-confirmation-note"><ShieldAlert />此处可填写待确认用例的人工覆盖原因；UI 入口、API 契约、环境或阈值等实际执行配置，需要通过新 Revision 补全后重新发布。</p><div><select value={mode} onChange={event => { setMode(event.target.value as TestExecutionMode); setSuiteId(''); setOverrideReasons({}) }}><option value="smoke">冒烟测试</option><option value="regression">回归测试</option><option value="full">全量测试</option><option value="custom">自定义测试</option></select>{mode !== 'full' && <select value={suiteId} onChange={event => { setSuiteId(event.target.value); setOverrideReasons({}) }}><option value="">选择当前正式用例版本的{modeLabel(mode)}套件</option>{eligibleSuites.map(item => <option key={item.id} value={item.id}>{item.name} V{item.version}</option>)}</select>}{mode === 'regression' && <input value={impacted} onChange={event => setImpacted(event.target.value)} placeholder="受影响测试用例 ID，逗号分隔（可选）" />}</div>
