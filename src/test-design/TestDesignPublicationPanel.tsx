  ...
  return <section className="tdw-panel tdw-publications">
    ...
    {!versions.length ? <div className="tdw-empty"><PackageCheck /><b>尚未发布正式用例库版本</b><span>从设计任务完成 Proposal 处置与 Coverage Audit 后发布。</span></div> : <div className="tdw-master-detail">
      ...
      {selected && <article className="tdw-release-detail">
        ...
        <section><h4>Coverage Audit</h4>{selected.publicationSummary ? <div className="tdw-audit-summary"><CheckCircle2 /><span><b>{selected.publicationSummary.coverageAudit.id}</b><small>Basis {selected.publicationSummary.coverageAudit.statistics.coveredBasis}/{selected.publicationSummary.coverageAudit.statistics.totalBasis} · Points {selected.publicationSummary.coverageAudit.statistics.coveredPoints}/{selected.publicationSummary.coverageAudit.statistics.totalPoints} · Blockers {selected.publicationSummary.coverageAudit.blockerCount}</small></span></div> : <p className="tdw-muted">该版本没有新版发布统计快照。</p>}</section>
