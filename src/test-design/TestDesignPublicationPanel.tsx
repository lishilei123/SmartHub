...
export function TestDesignPublicationPanel({ projectId, run, cases, versions, suites, handoffs, busy, onPublish, onHandoff }: PublicationProps) {
  ...
  return <section className="tdw-panel tdw-publications">
    ...
    {handoffs.length > 0 && <section className="tdw-handoff-history"><h3>Execution Handoff 记录</h3>{handoffs.map(item => <article key={item.id}><Send /><span><b>{item.mode} · {item.members.length} 个冻结成员 · {item.testDataSnapshot?.requirements.length ?? 0} 项数据需求</b><small>{item.testCaseLibraryVersionId}{item.suiteVersionId ? ` · ${item.suiteVersionId}` : ''}</small><code>{item.contentSha256}</code></span><time>{new Date(item.createdAt).toLocaleString('zh-CN')}</time></article>)}</section>}
  </section>
}
