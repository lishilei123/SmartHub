...
export const reviewCase = (projectVersionId: string, designId: string, runId: string, caseId: string, decision: 'approve', targetRevision: number, comment?: string) => request<TestDesignCase>(`${caseScope(projectVersionId, designId, runId, caseId)}/review-actions`, { method: 'POST', body: JSON.stringify({ decision, targetRevision, comment }) })
export const batchReviewCases = (projectVersionId: string, designId: string, runId: string, targets: Array<{ caseId: string; targetRevision: number }>, decision: 'approve') => request<TestDesignCase[]>(`${runScope(projectVersionId, designId, runId)}/test-cases/batch-review-actions`, { method: 'POST', body: JSON.stringify({ targets, decision }) })
