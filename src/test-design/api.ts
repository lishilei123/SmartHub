...
export const decideProposal = (projectVersionId: string, designId: string, runId: string, proposalId: string, input: { expectedVersion: number; decision: Exclude<CaseChangeDecision, 'pending'>; comment?: string }) => request<CaseChangeProposal>(`${runScope(projectVersionId, designId, runId)}/case-change-proposals/${encodeURIComponent(proposalId)}/decisions`, { method: 'POST', body: JSON.stringify(input) })
