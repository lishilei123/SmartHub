...
export type TestDesignIssueDecision = 'confirm' | 'resolve' | 'defer' | 'reject' | 'reopen'
export type TestDesignIssueActionInput = { expectedVersion: number; decision: TestDesignIssueDecision; comment?: string }
export const actOnFinding = (projectVersionId: string, designId: string, runId: string, findingId: string, input: TestDesignIssueActionInput) => request(`${runScope(projectVersionId, designId, runId)}/findings/${encodeURIComponent(findingId)}/actions`, { method: 'POST', body: JSON.stringify(input) })
export const actOnConfirmation = (projectVersionId: string, designId: string, runId: string, itemId: string, input: TestDesignIssueActionInput) => request<{ item: unknown; requiredAction: 're_audit' | 'resynthesize' }>(`${runScope(projectVersionId, designId, runId)}/confirmation-items/${encodeURIComponent(itemId)}/actions`, { method: 'POST', body: JSON.stringify(input) })
...
export const batchAcceptUnchangedReuseProposals = (projectVersionId: string, designId: string, runId: string, targets: Array<{ proposalId: string; expectedVersion: number }>) => request<{ acceptedCount: number; alreadyAcceptedCount: number }>(`${runScope(projectVersionId, designId, runId)}/case-change-proposals/batch-accept-unchanged-reuse`, { method: 'POST', body: JSON.stringify({ targets }) })
...
export const previewLegacyCaseMigration = (projectId: string, legacyTestCaseSetVersionId: string) => request<{ legacyTestCaseSetVersionId: string; status: 'ready' | 'needs_confirmation' | 'migrated'; previewSha256: string; items: Array<{ legacyCaseId: string; legacyRevision: number; suggestedLibraryCaseId: string; resolution: string; executionConfigurationStatus: 'ready' | 'needs_confirmation' | 'blocked'; executionConfigurationIssues: string[] }> }>(`${projectScope(projectId)}/test-case-library-migrations/${encodeURIComponent(legacyTestCaseSetVersionId)}/preview`)
