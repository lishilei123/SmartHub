...
export function TestCasePanel({ run, busy, onSubmit, onApprove, onResynthesize, onCreate, onEdit, onDelete, onReview }: Props) {
  ...
  return <section className="td2-card td2-cases">
    <header className="td2-section-head"><div><p className="td2-kicker">Test Case Design</p><h2>测试用例候选</h2><p>支持人工新增并直接关联 Requirement，结构化编辑执行步骤、单条审核与 Revision 追踪；批量操作只处理当前符合状态迁移条件的用例。</p></div><div className="td2-run-actions"><button className="td2-button ghost" disabled={busy} onClick={() => setEditor({ mode: 'create' })}><Plus />新增用例</button><button className="td2-button ghost" disabled={busy} onClick={onResynthesize}><RefreshCw />重新生成</button><button className="td2-button ghost" disabled={busy || !counts.draft} onClick={onSubmit}><Send />批量提交审核</button><button className="td2-button primary" disabled={busy || !counts.in_review} onClick={onApprove}><Check />批量批准</button></div></header>
...
function CaseRow({ testCase, requirementRefs, busy, expanded, onToggle, onEdit, onDelete, onReview }: { testCase: TestDesignCase; requirementRefs: string[]; busy: boolean; expanded: boolean; onToggle: () => void; onEdit: () => void; onDelete: () => void; onReview: (decision: ReviewDecision) => void }) {
  ...
  return <article className={expanded ? 'expanded' : ''}><button className="td2-case-summary" onClick={onToggle}><span className={`td2-review-state ${testCase.reviewState}`}>{reviewStateLabel(testCase.reviewState)}</span><div><b>{content.title}</b><small>{content.dimension} · {actualExecutionMethod(content)} · {readiness} · {content.priority} · Revision {testCase.currentRevision}</small></div><div className="td2-methods"><span>{actualExecutionMethod(content)}</span>{pending.length > 0 && <span className="needs_confirmation">待确认 {pending.length}</span>}</div><ChevronDown /></button>
    {expanded && <div className="td2-case-detail"><div><h4>目标</h4><p>{content.objective}</p></div><div><h4>前置条件</h4>{content.preconditions.length ? <ul>{content.preconditions.map(item => <li key={item}>{item}</li>)}</ul> : <p>无</p>}</div><section><header><b>{content.dimension} · {actualExecutionMethod(content)}</b><span className={readiness}>{readiness}</span></header><pre className="tdw-json">{JSON.stringify(content.executionSpec, null, 2)}</pre></section><section><h4>待确认项</h4>{pending.length ? <ul>{pending.map(item => <li key={item}>{item}</li>)}</ul> : <p>无</p>}</section><section><h4>Requirement 追溯 / 测试数据需求</h4><div className="tdw-chips">{requirementRefs.map(id => <code key={`requirement-${id}`}>{id}</code>)}{content.dataRequirementIds.map(id => <code key={`data-${id}`}>{id}</code>)}</div></section>
