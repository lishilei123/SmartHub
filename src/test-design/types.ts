...
export type AgentTestSpec = {
  ...
  argumentAssertions: Array<{ tool: string; path: string; operator: AgentValueAssertionOperator; expected?: unknown }>
  sequenceConstraints: Array<{ before: string; after: string }>
  businessAssertions: Array<{ path: string; operator: AgentValueAssertionOperator; expected?: unknown }>
  artifactAssertions: Array<{ name: string }>
  semanticAssertions: Array<{ criterion: string; expected: string }>
  safetyAssertions: Array<{ criterion: string; expected: string }>
  executionConstraints: { timeoutMs: number; maxSteps: number; repeatCount: number; maxCost?: number }
}
