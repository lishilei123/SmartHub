...
export function CoverageAuditPanel({ run, busy, onAudit }: { run: TestDesignWorkflowRun; busy: boolean; onAudit: () => void }) {
  ...
  return <section className="td2-card td2-audit">
    <header className="td2-section-head"><div><p className="td2-kicker">Server Coverage Audit</p><h2>确定性覆盖检查</h2><p>Coverage 只由 TestCase v3 的显式 Requirement 引用计算；扩展测试不隐含覆盖。</p></div><button className={`td2-button ${stale ? 'primary' : 'ghost'}`} disabled={busy || !run.testCases.length} onClick={onAudit}><RefreshCw />{stale ? '重新检查当前内容' : '重新检查'}</button></header>
    {!audit ? <div className="td2-empty-compact"><Server /><span>等待 TestCase 生成后执行服务端审计</span></div> : <>
      <div className={`td2-audit-state ${pass ? 'pass' : stale ? 'stale' : 'blocked'}`}>{pass ? <CheckCircle2 /> : <AlertTriangle />}<div><b>{pass ? 'Coverage Audit 已通过' : stale ? '当前结果已失效' : `存在 ${audit.blockers.length} 个阻断项`}</b><small>{pass ? '所有正式 Requirement 均有显式 TestCase 引用。' : '按下方确定性问题修改 TestCase 后重新检查。'}</small></div></div>
      <div className="td2-audit-kpis"><span><small>Requirement 覆盖</small><b>{audit.statistics.coveredBasis}/{audit.statistics.totalBasis}</b></span><span><small>当前 TestCase</small><b>{audit.statistics.totalCases}</b></span><span><small>Blockers</small><b>{audit.blockers.length}</b></span><span><small>Advisories</small><b>{audit.advisories.length}</b></span></div>
