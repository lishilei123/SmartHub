...
function ReviewAndPublishBar({ run, versions, busy, blockers, onOpenPublish }: { run: TestDesignWorkflowRun; versions: ReturnType<typeof useTestDesign>['libraryVersions']; busy: boolean; blockers: string[]; onOpenPublish: () => void }) {
  ...
  return <section className="tdw-review-publish"><div className="tdw-review-summary"><span><small>当前用例数量</small><b>{activeCases.length}</b></span><span><small>已审核数量</small><b>{approved}/{activeCases.length}</b></span><span><small>Requirement 覆盖</small><b>{coverage}</b></span><span><small>待处理问题</small><b>{buildPendingIssues(run).length}</b></span><span><small>执行就绪问题</small><b>{executionIssues}</b></span></div><div className="tdw-review-publish-action">{published ? <div className="tdw-published-result"><CheckCircle2 /><span><b>已发布正式用例版本</b><small>{published.name} · V{published.version}</small></span></div> : <><div>{blockers.length ? <small>{blockers[0]}</small> : <small>所有条件已满足，点击后填写正式版本名称。</small>}</div><button className="td2-button primary" disabled={busy || blockers.length > 0} onClick={onOpenPublish}><Rocket />确认并发布</button></>}</div></section>
}
...
function buildPendingIssues(run: TestDesignWorkflowRun): PendingIssue[] {
  ...
  const audit = run.coverageAudits.at(-1)
  const activeCases = run.testCases.filter(item => !item.tombstonedAt)
  if (activeCases.length && (!audit || audit.status === 'stale')) issues.push({ key: 'audit', category: '覆盖检查', title: '覆盖检查需要重新执行', description: '测试用例已经发生变化，请重新执行覆盖检查。', severity: 'blocker', action: 'audit' })
...
function deriveStages(run: TestDesignWorkflowRun | null, versions: ReturnType<typeof useTestDesign>['libraryVersions']) {
  ...
  const audit = run.coverageAudits.at(-1)
  const reviewDone = activeCases.length > 0 && activeCases.every(item => item.reviewState === 'approved')
  const coverageDone = audit?.status === 'valid' && !audit.blockers.some(item => item.resolution !== 'execution_handoff')
  const published = versions.find(item => item.sourceRunId === run.id)
  return [
    { label: 'AI 生成', detail: generation?.status === 'succeeded' ? `已生成 ${activeCases.length} 条用例` : ['failed', 'cancelled'].includes(run.status) ? runStatusLabel(run.status) : 'PlanningAgent 正在生成', state: generation?.status === 'succeeded' ? 'complete' : ['failed', 'cancelled'].includes(run.status) ? 'blocked' : 'active' },
