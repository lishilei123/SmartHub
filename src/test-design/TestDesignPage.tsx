...
const entryFromRoute = (value: string | null): Entry => value === 'library' || value === 'suites' || value === 'releases' ? value : 'designs'
...
export function TestDesignPage({ projectVersion, onManageVersions, notify, embedded = false, linkedDesignId, linkedRunId, initialCreate = false }: { projectVersion: ProjectVersion | null; onManageVersions: () => void; notify: Notify; embedded?: boolean; linkedDesignId?: string; linkedRunId?: string; initialCreate?: boolean }) {
  ...
  const [entry, setEntry] = useState<Entry>(() => entryFromRoute(routedEntry)); const model = useTestDesign(projectVersion?.id, notify, entry); const [tab, setTab] = useState<DesignTab>('details'); const [creating, setCreating] = useState(initialCreate); const autoOpeningId = useRef(''); const linkedOpeningId = useRef(''); const linkedOpenedTargetId = useRef('')
  useEffect(() => { const syncEntry = () => setEntry(entryFromRoute(new URL(window.location.href).searchParams.get('testDesignEntry'))); window.addEventListener('popstate', syncEntry); return () => window.removeEventListener('popstate', syncEntry) }, [])
  ...
  if (embedded) return <section className="tdw-embedded" aria-label="测试用例">
    ...
    {entry === 'releases' && <div className="tdw-content"><TestDesignPublicationPanel projectId={projectId} cases={model.libraryCases} versions={model.libraryVersions} busy={Boolean(model.busy)} /></div>}
  </section>
...
function DesignWorkspace({ creating, tab, setTab, model, onCreateDone }: { creating: boolean; tab: DesignTab; setTab: (tab: DesignTab) => void; model: ReturnType<typeof useTestDesign>; onCreateDone: () => void }) {
  ...
  if (!model.design) return <div className="tdw-content"><section className="tdw-welcome"><div><Bot /><span><p>PlanningAgent</p><h2>等待正式需求</h2><p>正式需求版本发布后，Service 会自动创建测试设计并直接生成多场景测试用例与覆盖检查。</p></span></div><div className="tdw-pipeline">{['需求基线', '必要时人工澄清', 'AI 测试用例生成', '覆盖检查', '最终人工审核', '发布正式用例'].map((item, index) => <span key={item}><i>{index + 1}</i>{item}</span>)}</div></section></div>
  const run = model.run
  if (!run) return <div className="tdw-content tdw-review-workspace"><section className="td2-card td2-empty"><Bot /><h2>尚未开始 AI 生成</h2><p>启动后，PlanningAgent 会基于已绑定的正式需求生成本次测试用例。</p><button className="td2-button primary" disabled={Boolean(model.busy)} onClick={() => void model.startRun().catch(() => undefined)}>启动 AI 生成</button></section><AdvancedInformation tab={tab} setTab={setTab} model={model} run={null} /></div>
  ...
  return <div className="tdw-content tdw-review-workspace">
    ...
    <TestCasePanel run={run} busy={Boolean(model.busy)} createRequest={createRequest} focusRequest={caseFocusRequest} onBatchApprove={targets => model.reviewCases(targets)} onResynthesize={() => model.resynthesize()} onCreate={content => model.createCase(content)} onEdit={(caseId, content, reason) => model.editCase(caseId, content, reason)} onDelete={caseId => model.removeCase(caseId)} onReview={(caseId, decision, revision, comment) => model.reviewCase(caseId, decision, revision, comment)} />
    <ReviewAndPublishBar run={run} versions={model.libraryVersions} busy={Boolean(model.busy)} blockers={publicationBlockers} onOpenPublish={() => { void model.loadPublicationAssets().then(loaded => { if (loaded) setPublishOpen(true) }).catch(() => undefined) }} />
    <AdvancedInformation tab={tab} setTab={setTab} model={model} run={run} />
    {publishOpen && <div className="tdw-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setPublishOpen(false) }}><section className="tdw-modal wide tdw-workspace-modal" role="dialog" aria-modal="true" aria-label="确认并发布"><header><div><Rocket /><span><b>确认并发布</b><small>发布成功后进入正式测试用例库。</small></span></div><button aria-label="关闭" onClick={() => setPublishOpen(false)}><X /></button></header><TestDesignPublicationPanel projectId={model.inputs!.projectVersion.projectId} run={run} cases={model.libraryCases} versions={model.libraryVersions} busy={Boolean(model.busy)} onPublish={name => model.publish(name)} /></section></div>}
  </div>
}
...
function ReviewAndPublishBar({ run, versions, busy, blockers, onOpenPublish }: { run: TestDesignWorkflowRun; versions: ReturnType<typeof useTestDesign>['libraryVersions']; busy: boolean; blockers: string[]; onOpenPublish: () => void }) {
  ...
  return <section className="tdw-review-publish"><div className="tdw-review-summary"><span><small>Candidate Delta</small><b>{activeCases.length}</b></span><span><small>Effective Case Set</small><b>{run.effectiveCaseCount ?? audit?.statistics.totalCases ?? activeCases.length}</b></span><span><small>已审核 Candidate</small><b>{approved}/{activeCases.length}</b></span><span><small>需求追溯覆盖率</small><b>{coverage}</b></span><span><small>待处理问题</small><b>{buildPendingIssues(run).length}</b></span><span><small>执行就绪问题</small><b>{executionIssues}</b></span></div><div className="tdw-review-publish-action">{published ? <div className="tdw-published-result"><CheckCircle2 /><span><b>已发布正式用例版本</b><small>{published.name} · V{published.version}</small></span></div> : <><div>{blockers.length ? <small>{blockers[0]}</small> : <small>所有条件已满足，点击后填写正式版本名称。</small>}</div><button className="td2-button primary" disabled={busy || blockers.length > 0} onClick={onOpenPublish}><Rocket />确认并发布</button></>}</div></section>
}

function AdvancedInformation({ tab, setTab, model, run }: { tab: DesignTab; setTab: (tab: DesignTab) => void; model: ReturnType<typeof useTestDesign>; run: TestDesignWorkflowRun | null }) {
