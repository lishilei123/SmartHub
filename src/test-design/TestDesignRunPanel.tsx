...
export function TestDesignRunPanel({ design, run, busy, onRefresh, onStartRun }: { design: TestDesign; run: TestDesignWorkflowRun | null; busy: boolean; onRefresh: () => void; onStartRun: () => void }) {
  ...
  const readFiles = [...new Map(executions.flatMap(item => item.execution.inputDeliveryManifest?.toolReads ?? []).map(read => [`${read.sourceScope ?? 'workspace'}:${read.relativePath}`, read])).values()]
  ...
  return <section className="td2-run-grid">
    <div className="td2-card td2-run-main">
      ...
      <div className="td2-workspace-reads"><header><Database /><span><b>PlanningAgent 实际读取文件</b><small>{readFiles.length} 个 · currentInputRefs 不是读取白名单</small></span></header>{readFiles.length ? <div>{readFiles.map(read => <article key={`${read.sourceScope ?? 'workspace'}:${read.relativePath}`}><code>[{read.sourceScope ?? 'workspace'}]</code><span>{read.relativePath}</span></article>)}</div> : <p>尚无 workspace.read_file 记录。</p>}</div>
