...
export function TestDesignCreatePanel({ inputs, busy, onCreate, onCancel }: { inputs: TestDesignInputCandidates; busy: boolean; onCreate: (input: CreateTestDesignInput) => Promise<void> | void; onCancel: () => void }) {
  ...
  const latestInheritedLibrary = inputs.projectVersion.inheritsSourceAssets ? inputs.testCaseLibraryVersions[0] : undefined
  const [historyMode, setHistoryMode] = useState<'latest_library' | 'library_version' | 'suite_version' | 'none'>(latestInheritedLibrary ? 'latest_library' : 'none')
  const [historyVersionId, setHistoryVersionId] = useState(latestInheritedLibrary?.id ?? '')
  ...
  const submit = () => onCreate({
    ...
    historicalLibrarySelection: historyMode === 'latest_library' ? { mode: historyMode } : historyMode === 'library_version' ? { mode: historyMode, testCaseLibraryVersionId: historyVersionId || (inputs.testCaseLibraryVersions[0]?.id ?? '') } : historyMode === 'suite_version' ? { mode: historyMode, suiteVersionId: historyVersionId || (inputs.historicalTestSuites[0]?.id ?? '') } : { mode: 'none' },
  })
  ...
  return <section className="td2-create td2-card">
    ...
    <div className="td2-history-source"><div><b>历史用例策略</b><small>{inputs.projectVersion.inheritsSourceAssets ? '仅可选择来源版本中已明确继承的冻结用例库或测试套件；运行时再次冻结快照。' : '当前版本未明确继承来源版本；不会加载任何需求外的历史用例或测试资产。'}</small></div><select value={historyMode} onChange={event => { const mode = event.target.value as typeof historyMode; setHistoryMode(mode); setHistoryVersionId(mode === 'library_version' ? inputs.testCaseLibraryVersions[0]?.id ?? '' : mode === 'suite_version' ? inputs.historicalTestSuites[0]?.id ?? '' : '') }}><option value="none">不使用历史用例</option><option value="latest_library" disabled={!inputs.projectVersion.inheritsSourceAssets || !inputs.testCaseLibraryVersions.length}>来源版本最新正式用例库（默认）</option><option value="library_version" disabled={!inputs.projectVersion.inheritsSourceAssets || !inputs.testCaseLibraryVersions.length}>指定来源用例库版本</option><option value="suite_version" disabled={!inputs.projectVersion.inheritsSourceAssets || !inputs.historicalTestSuites.length}>指定来源测试套件</option></select>{historyMode === 'library_version' && <select value={historyVersionId} onChange={event => setHistoryVersionId(event.target.value)}>{inputs.testCaseLibraryVersions.map(item => <option key={item.id} value={item.id}>V{item.version} · {item.name} · {item.memberCount} 条</option>)}</select>}{historyMode === 'suite_version' && <select value={historyVersionId} onChange={event => setHistoryVersionId(event.target.value)}>{inputs.historicalTestSuites.map(item => <option key={item.id} value={item.id}>{item.suiteType} · {item.name} V{item.version}</option>)}</select>}{latestInheritedLibrary && <article className="td2-inherited-library"><b>默认继承来源版本：{inputs.projectVersion.sourceProjectVersionName ?? inputs.projectVersion.sourceProjectVersionId}</b><small>正式用例库：V{latestInheritedLibrary.version} / {latestInheritedLibrary.name} / {latestInheritedLibrary.memberCount} 条 / {new Date(latestInheritedLibrary.publishedAt).toLocaleString('zh-CN')}</small></article>}</div>
