...
export function TestDesignCreatePanel({ inputs, busy, onCreate, onCancel }: { inputs: TestDesignInputCandidates; busy: boolean; onCreate: (input: CreateTestDesignInput) => Promise<void> | void; onCancel: () => void }) {
  ...
  const [historyMode, setHistoryMode] = useState<'library_version' | 'suite_version' | 'none'>('none')
  ...
  const submit = () => onCreate({
    ...
    historicalLibrarySelection: historyMode === 'library_version' ? { mode: historyMode, testCaseLibraryVersionId: historyVersionId || (inputs.testCaseLibraryVersions[0]?.id ?? '') } : historyMode === 'suite_version' ? { mode: historyMode, suiteVersionId: historyVersionId || (inputs.historicalTestSuites[0]?.id ?? '') } : { mode: 'none' },
  })
