...
export async function createAgentUnderTest(id: string, input: { name: string; endpoint: string; protocol: 'http' | 'sse'; authenticationConfig: AgentUnderTest['versions'][number]['authenticationConfig']; requestMapping: AgentUnderTest['versions'][number]['requestMapping']; responseMapping: AgentUnderTest['versions'][number]['responseMapping']; documentationRefs: string[] }) { return (await request<AgentUnderTest>(`${projectScope(id)}/agents-under-test`, { method: 'POST', body: JSON.stringify(input) })).value }
