...
export type ExecutionReadiness = {
  ...
  worker: { ready: boolean; reason?: string; activeWorkers: number }
  agents: { ready: boolean; agents: Array<{ agentKey: string; ready: boolean; reason?: string }> }
  runner: { ready: boolean; snapshot: { kind: 'agent'; runnerVersion: 'agent-runner/v2' } }
...
export type ExecutionRun = {
  ...
  runner: { kind: 'agent'; runnerVersion: 'agent-runner/v1' | 'agent-runner/v2' }
...
export type AgentCaseRun = {
  id: string; executionAttemptOrdinal: number; repeatOrdinal: number; status: 'PASS' | 'FAIL' | 'NOT_EVALUABLE' | 'ERROR'; actualOutput?: unknown
  assertionResults: AgentAssertionResult[]; evaluationResults: AgentEvaluationResult[]; traceEvents: TraceEvent[]; evidenceRefs: string[]
  evidenceCoverage: Record<string, 'complete' | 'partial' | 'unavailable'>; failureFacts: Array<{ code: string; message: string; evidenceRefs: string[] }>
...
export type AgentExecutionAggregateResult = {
  taskId: string; runId: string; executionAttemptOrdinal: number; status: AgentCaseRun['status']; caseRuns: AgentCaseRun[]
...
export type ExecutionTaskDetail = { run: ExecutionRun; task: ExecutionTask; agentExecutionResult?: AgentExecutionAggregateResult; agentExecutionAttempts: AgentExecutionAggregateResult[] }
