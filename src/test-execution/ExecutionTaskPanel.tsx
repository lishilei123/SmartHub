    ...
    <section className="te-card te-task-detail">
      ...
      {task && <>
        ...
        {tab === 'scripts' && <div className="te-script-panel">
          <div className="te-revision-list">{task.value.scriptRevisions.map(item => <article key={item.id}><Code2 /><span><b>Revision {item.revision} · {item.source}</b><small>{item.repairReason ?? (item.cacheSourceRevisionId ? `缓存来源 ${shortId(item.cacheSourceRevisionId)}` : '初始脚本')} · {item.sourceArtifacts.length} files</small><code title={item.package.packageSha256}>{item.package.packageSha256}</code></span></article>)}</div>
