...
export function TestExecutionPage({
  ...
  useEffect(() => {
    if (restoredRun.current || !projectVersion || !model.runs.length) return
    restoredRun.current = true
    const runId = new URL(window.location.href).searchParams.get('executionRunId')
    if (runId && model.runs.some(item => item.id === runId)) {
      void model.openRun(runId).catch(cause => notify(messageOf(cause), 'error'))
    }
  }, [model.openRun, model.runs, notify, projectVersion])
