...
const agentConfigurationMetadata: Record<AgentConfigurationAgentKey, { label: string; identifier: string; sceneLabel: string; protocolLabel: string; publishTarget: string; runtimeToolIds: string[]; exactCapabilities: boolean }> = {
  planning: { label: 'PlanningAgent', identifier: 'PlanningAgent', sceneLabel: '测试策划', protocolLabel: 'Planning + Project Workspace v1', publishTarget: '新的需求分析与测试设计任务', runtimeToolIds: ['workspace.list_directory', 'workspace.find_files', 'workspace.grep_files', 'workspace.read_file', 'knowledge.search', 'knowledge.read_chunk', 'skill.activate', 'requirement-analysis.submit_result', 'requirement-repair.submit_result', 'requirement-release.submit_result', 'test_design_points.submit_result', 'test_design_cases.submit_result', 'test_design_repair.submit_result'], exactCapabilities: false },
  testScript: { label: '测试脚本 Agent', identifier: 'TestScriptAgent', sceneLabel: '测试执行', protocolLabel: '测试脚本生成 v1', publishTarget: '新测试执行运行', runtimeToolIds: ['workspace.list_directory', 'workspace.find_files', 'workspace.grep_files', 'workspace.read_file', 'skill.activate', 'test_script.submit_result'], exactCapabilities: true },
  failureAnalysis: { label: '失败分析 Agent', identifier: 'FailureAnalysisAgent', sceneLabel: '测试执行', protocolLabel: '失败分析 v1', publishTarget: '新测试执行诊断', runtimeToolIds: ['workspace.list_directory', 'workspace.find_files', 'workspace.grep_files', 'workspace.read_file', 'skill.activate', 'failure_analysis.submit_result'], exactCapabilities: true },
