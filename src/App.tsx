...
import { loadExecutionInfrastructureConfiguration, publishExecutionInfrastructureConfiguration, type ExecutionEnvironmentProfile, type ExecutionInfrastructureVersion, type ExecutionRunnerConfiguration } from './test-execution-infrastructure-api'
import './planning.css'
  ...
  const remove = async () => {
    ...
    try {
      ...
      const workspaceCleanup = deleted.workspaceCleanupTaskIds.length
        ...
        : ''
      notify(`项目版本 ${deleted.name} 已删除，同时移除 ${deleted.deletedBindings} 条需求绑定、${deleted.deletedAnalysisRuns} 条需求分析运行和 ${deleted.deletedTestDesigns} 个测试设计${workspaceCleanup}。`)
...
function SystemSettings({ knowledgeBaseId, notify, addAudit }: { knowledgeBaseId: string; notify: Notify; addAudit: (entry: string) => void }) {
  const items = [
    ...
    { name: '执行基础设施', desc: '执行环境、OCI Runner 与密钥引用', icon: Server, group: '资源与集成' },
  ...
  return <div className={`settings-layout ${collapsed ? 'directory-collapsed' : ''}`}><aside className={`card settings-directory ${collapsed ? 'collapsed' : ''}`}><div className="settings-dir-head"><b>配置目录</b><button className="icon-btn" title={collapsed ? '展开配置目录' : '收起配置目录'} aria-label={collapsed ? '展开配置目录' : '收起配置目录'} onClick={() => setCollapsed(value => !value)}>{collapsed ? <PanelLeftOpen /> : <PanelLeftClose />}</button></div>{['AI 能力', '资源与集成', '安全与治理'].map(group => <div className="settings-group" key={group}><p>{group}</p>{items.map((item, index) => item.group === group && <button key={item.name} className={selected === index ? 'active' : ''} onClick={() => setSelected(index)}><item.icon /><span><b>{item.name}</b><small>{item.desc}</small></span><ChevronRight /></button>)}</div>)}</aside>
    <section className="card settings-editor">{selected !== 1 && <div className="settings-editor-head"><div className="setting-symbol"><CurrentIcon /></div><div><h2>{current.name}</h2><p>{current.desc}{selected === 2 && configVersion ? ` · 配置 V${configVersion}` : ''}</p></div>{selected !== 0 && <><Badge tone={dirty ? 'orange' : requiresRebuild && selected === 2 ? 'orange' : 'green'}>{dirty ? '有未保存更改' : requiresRebuild && selected === 2 ? '待重建' : '已保存'}</Badge><button className="btn primary" disabled={!dirty} onClick={() => void save()}><Check />保存配置</button></>}</div>}<div className="settings-editor-scroll" ref={editorScrollRef}>
      ...
      {selected === 3 && <ExecutionInfrastructureSettings notify={notify} addAudit={addAudit} />}
...
function ExecutionInfrastructureSettings({ notify, addAudit }: { notify: Notify; addAudit: (entry: string) => void }) {
  const blankRunner = (): ExecutionRunnerConfiguration => ({ containerRuntime: 'docker', runnerVersion: '', playwrightVersion: '', imageReference: '', imageDigest: '', entrypoint: '/opt/smarthub/run-playwright', workingRoot: '' })
  const [active, setActive] = useState<ExecutionInfrastructureVersion | null>(null)
  const [environments, setEnvironments] = useState<ExecutionEnvironmentProfile[]>([])
  const [runner, setRunner] = useState<ExecutionRunnerConfiguration>(blankRunner)
  const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false)
  const load = useCallback(async () => { setLoading(true); try { const state = await loadExecutionInfrastructureConfiguration(); setActive(state.activeVersion); setEnvironments(state.activeVersion?.environments ?? []); setRunner(state.activeVersion?.runner ?? blankRunner()) } catch (error) { notify(error instanceof Error ? error.message : '执行基础设施配置读取失败', 'error') } finally { setLoading(false) } }, [notify])
  useEffect(() => { void load() }, [load])
  const updateEnvironment = (index: number, patch: Partial<ExecutionEnvironmentProfile>) => setEnvironments(current => current.map((item, position) => position === index ? { ...item, ...patch } : item))
  const save = async () => { setSaving(true); try { const readyRunner = runner.runnerVersion.trim() && runner.playwrightVersion.trim() && runner.imageReference.trim() && runner.imageDigest.trim(); const published = await publishExecutionInfrastructureConfiguration({ expectedActiveVersion: active?.version ?? null, environments, ...(readyRunner ? { runner: { ...runner, entrypoint: runner.entrypoint?.trim() || undefined, workingRoot: runner.workingRoot?.trim() || undefined } } : {}) }); setActive(published); addAudit(`发布测试执行基础设施 V${published.version}`); notify(`执行基础设施 V${published.version} 已发布；后续 Run 将冻结该版本。`); await load() } catch (error) { notify(error instanceof Error ? error.message : '执行基础设施发布失败', 'error') } finally { setSaving(false) } }
  if (loading) return <div className="agent-configuration-status"><RefreshCw /><div><b>正在读取执行基础设施</b><span>环境与 OCI Runner 配置保存在服务端版本中。</span></div></div>
  return <div className="settings-form execution-infrastructure-settings"><FormSection title="登记执行环境" desc="只有登记地址才能创建执行 Run；密钥只填写服务器环境变量引用。"><div className="execution-infrastructure-toolbar"><span>{environments.length} 个环境 · {active ? `当前 V${active.version}` : '尚未发布'}</span><button type="button" className="btn ghost" onClick={() => setEnvironments(current => [...current, { environmentId: '', name: '', baseUrl: '', targets: [{ protocol: 'https', host: '', port: 443 }], networkName: '', secretEnvironmentVariables: {} }])}><Plus />添加环境</button></div>{environments.map((environment, index) => <section className="execution-environment-editor" key={`${environment.environmentId}-${index}`}><header><b>环境 {index + 1}</b><button type="button" className="icon-btn danger-text" aria-label={`删除环境 ${index + 1}`} onClick={() => setEnvironments(current => current.filter((_, position) => position !== index))}><Trash2 /></button></header><div className="execution-infrastructure-grid"><label>环境标识<input value={environment.environmentId} onChange={event => updateEnvironment(index, { environmentId: event.target.value })} placeholder="sit-web" /></label><label>名称<input value={environment.name} onChange={event => updateEnvironment(index, { name: event.target.value })} placeholder="SIT Web" /></label><label className="wide">被测地址<input value={environment.baseUrl} onChange={event => updateEnvironment(index, { baseUrl: event.target.value })} placeholder="https://sit.example.com/" /></label><label>OCI 网络<input value={environment.networkName} onChange={event => updateEnvironment(index, { networkName: event.target.value })} placeholder="smarthub-sit" /></label><label>允许主机<input value={environment.targets[0]?.host ?? ''} onChange={event => updateEnvironment(index, { targets: [{ protocol: environment.targets[0]?.protocol ?? 'https', host: event.target.value, port: environment.targets[0]?.port ?? 443 }] })} placeholder="sit.example.com" /></label><label>端口<input type="number" min="1" max="65535" value={environment.targets[0]?.port ?? 443} onChange={event => updateEnvironment(index, { targets: [{ protocol: environment.targets[0]?.protocol ?? 'https', host: environment.targets[0]?.host ?? '', port: Number(event.target.value) }] })} /></label></div></section>)}</FormSection><FormSection title="OCI Playwright Runner" desc="镜像必须固定 digest；Runner 和网络会在创建/执行时由服务端验证。"><div className="execution-infrastructure-grid"><label>运行时<select value={runner.containerRuntime} onChange={event => setRunner(current => ({ ...current, containerRuntime: event.target.value as 'docker' | 'podman' }))}><option value="docker">Docker</option><option value="podman">Podman</option></select></label><label>Runner 版本<input value={runner.runnerVersion} onChange={event => setRunner(current => ({ ...current, runnerVersion: event.target.value }))} /></label><label>Playwright 版本<input value={runner.playwrightVersion} onChange={event => setRunner(current => ({ ...current, playwrightVersion: event.target.value }))} /></label><label className="wide">镜像引用<input value={runner.imageReference} onChange={event => setRunner(current => ({ ...current, imageReference: event.target.value }))} placeholder="registry.example.com/smarthub/runner" /></label><label className="wide">镜像 Digest<input value={runner.imageDigest} onChange={event => setRunner(current => ({ ...current, imageDigest: event.target.value }))} placeholder="sha256:…" /></label><label>入口<input value={runner.entrypoint ?? ''} onChange={event => setRunner(current => ({ ...current, entrypoint: event.target.value }))} /></label><label>工作目录（可选）<input value={runner.workingRoot ?? ''} onChange={event => setRunner(current => ({ ...current, workingRoot: event.target.value }))} /></label></div></FormSection><div className="execution-infrastructure-actions"><span>每次保存发布新版本；历史 Run 继续使用冻结版本。</span><button type="button" className="btn primary" disabled={saving} onClick={() => void save()}><Check />{saving ? '发布中…' : '发布基础设施配置'}</button></div></div>
}
...
function PlanningAgentSettings({ profile, error, modelSources, agentDraft, onRetry }: { profile: PlanningAgentProfile | null; error: string; modelSources: GenerativeSourceDraft[]; agentDraft: Record<AgentConfigurationAgentKey, AgentConfigurationAgentDraft>; onRetry: () => void }) {
  ...
  return <section className="planning-agent-profile">
    ...
    <section className="planning-agent-stages"><header><Activity /><span><b>Workflow Stage 边界</b><small>Workflow 固定业务 Gate、Allowed Tools、Submit Tool、Schema 与 Reviewer；Skill 始终来自 PlanningAgent 的 Enabled Skills。</small></span></header><div>{profile.stageProfiles.map(stage => <article key={stage.stage}><header><b>{stage.stage}</b><Badge tone={stage.humanGate ? 'orange' : 'gray'}>{stage.humanGate ? 'Human Gate' : stage.agentKey}</Badge></header><dl><div><dt>Tools</dt><dd>{stage.allowedToolIds.join(' · ') || '—'}</dd></div><div><dt>Submit / Schema</dt><dd>{stage.submitToolId ?? '—'} · {stage.resultSchemaVersion ?? '—'}</dd></div><div><dt>Reviewer</dt><dd>{stage.reviewers.join(' · ') || '—'}</dd></div></dl></article>)}</div></section>
  </section>
}
