...
const agentConfigurationGroups: Array<{ label: string; agentKeys: AgentConfigurationAgentKey[] }> = [
  ...
  { label: '测试执行', agentKeys: ['failureAnalysis'] },
]
...
const pageMeta: Record<PageKey, { title: string; desc: string }> = {
  ...
  execution: { title: '测试执行', desc: '基于当前正式 Agent Test 用例库与 AgentRunner 的正式执行工作台' },
...
function App() {
  ...
  const updateRoute = useCallback((next: { page?: PageKey; projectVersionId?: string; planningTab?: PlanningTab; resetAnalysisContext?: boolean }, mode: 'push' | 'replace' = 'push') => {
    ...
    if (next.resetAnalysisContext) ['analysisId', 'runId', 'view', 'findingId', 'evidenceId', 'testDesignId', 'workflowRunId', 'executionRunId', 'executionTaskId', 'testDesignEntry', 'libraryCaseId', 'testCaseId', 'reportRunId', 'tab', 'assetView', 'planningTab'].forEach(key => url.searchParams.delete(key))
