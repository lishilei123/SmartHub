...
function ResourceMetadata({ resource, catalog }: { resource: AiResource; catalog: AiResourceCatalog | null }) {
  ...
  if (resource.kind === 'skill') return <footer><span>{resource.package ? `ZIP · ${resource.package.fileCount} 个文件 · ${formatBytes(resource.package.unpackedBytes)}` : '手动入口'}</span><span>{resource.toolIds.length} 个依赖工具</span>{resource.runtime?.scripts.length ? <span>{resource.runtime.scripts.length} 个允许脚本</span> : null}{resource.runtime?.network ? <span>{resource.runtime.network.allowedOrigins.length} 个网络 Origin</span> : null}<span>{resource.tags.join(' · ') || '未设置标签'}</span><span title={resource.package?.contentSha256 ?? resource.entrypoint}>{resource.package ? `SHA-256 ${resource.package.contentSha256.slice(0, 12)}…` : resource.entrypoint}</span></footer>
