...
function AiResourceManagement({ kind, notify }: { kind: AiResourceKind; notify: Notify }) {
  ...
  const openEdit = (resource: AiResource) => setEditor({
    ...
    entrypoint: resource.kind === 'skill' ? resource.entrypoint : '', skillMode: resource.kind === 'skill' && resource.package ? 'zip' : 'entrypoint', packageFile: null, package: resource.kind === 'skill' ? resource.package : undefined, runtime: resource.kind === 'skill' ? resource.runtime : undefined, toolIds: resource.kind === 'tool' ? [] : [...resource.toolIds], tags: resource.kind === 'skill' ? resource.tags.join(', ') : '',
...
function ResourceMetadata({ resource, catalog }: { resource: AiResource; catalog: AiResourceCatalog | null }) {
  ...
  if (resource.kind === 'skill') return <footer><span>{resource.package ? `ZIP · ${resource.package.fileCount} 个文件 · ${formatBytes(resource.package.unpackedBytes)}` : '手动入口'}</span><span>{resource.toolIds.length} 个外部依赖工具</span><span>{skillRuntimePermissionSummary(resource)}</span><span>{resource.tags.join(' · ') || '未设置标签'}</span><span title={resource.package?.contentSha256 ?? resource.entrypoint}>{resource.package ? `SHA-256 ${resource.package.contentSha256.slice(0, 12)}…` : resource.entrypoint}</span></footer>
...
function skillRuntimePermissionSummary(skill: Pick<SkillResource, 'runtime'>) {
  const permissions = [skill.runtime?.scripts.length ? `${skill.runtime.scripts.length} 个 PowerShell 脚本` : '', skill.runtime?.network ? `${skill.runtime.network.allowedOrigins.length} 个网络 Origin` : ''].filter(Boolean)
  return permissions.length ? `运行权限：${permissions.join('、')}` : '无脚本或网络运行权限'
}
function agentSkillSummary(skill: SkillResource) {
  const details = [skill.toolIds.length ? `外部依赖：${skill.toolIds.join('、')}` : '', skillRuntimePermissionSummary(skill)].filter(Boolean)
  return details.join('；')
}
...
function PromptAgentSettings({ draft, notify, agentDraft, updateAgent, configuration, configurationError, modelSourcesState, modelSourcesError, onRetryConfiguration, onRetryModelSources, publishing, onPublish }: {
  ...
  const isTechnical = selectedAgent === 'technicalSolutionExtraction' || selectedAgent === 'technicalSolutionReview'
  const agentLabel = isExtraction ? '需求点提取 Agent' : isReview ? '需求评审 Agent' : selectedAgent === 'technicalSolutionExtraction' ? '技术方案提取 Agent' : selectedAgent === 'technicalSolutionReview' ? '技术方案评审 Agent' : '评审问答 Agent'
  const agentIdentifier = isExtraction ? 'RequirementPointExtractionAgent' : isReview ? 'RequirementReviewAgent' : selectedAgent === 'technicalSolutionExtraction' ? 'TechnicalSolutionExtractionAgent' : selectedAgent === 'technicalSolutionReview' ? 'TechnicalSolutionReviewAgent' : 'ReviewQaAgent'
  ...
  const stageRuntimeToolIds = new Set(isExtraction ? ['knowledge.search', 'knowledge.read_chunk', 'requirement-points.submit_result'] : isReview ? ['review.submit_result'] : ['review.answer_submit'])
