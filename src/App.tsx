...
import {
  ...
  type EmbeddingSourceDraft, type GenerativeSourceDraft, type SettingsDraft, type Version,
} from './prototype-data'
...
type GenerativeSourceEditor = { name: string; providerType: GenerativeSourceDraft['providerType']; endpointRef: string; credentialRef: string; modelName: string }
...
function ModelManagementSettings({ draft, update, notify }: { draft: SettingsDraft; update: <K extends keyof SettingsDraft>(key: K, value: SettingsDraft[K]) => void; notify: Notify }) {
  ...
  const [probingSource, setProbingSource] = useState('')
  ...
  const deleteSource = (source: GenerativeSourceDraft) => {
    ...
    setSources(remaining); update('fallbackModelIds', draft.fallbackModelIds.filter(id => !removedIds.has(id)))
    if (removedIds.has(draft.mainModel)) update('mainModel', remaining.flatMap(item => item.models).find(model => model.enabled)?.id ?? '')
  }
  const saveSource = () => {
    if (!sourceEditor) return
    const name = sourceEditor.name.trim(); const modelName = sourceEditor.modelName.trim()
    if (!name || !sourceEditor.endpointRef.trim() || !sourceEditor.credentialRef.trim() || !modelName) { notify('请完整填写来源、引用和首个模型。', 'warning'); return }
    const now = Date.now(); const source: GenerativeSourceDraft = { id: `source-${now}`, name, providerType: sourceEditor.providerType, endpointRef: sourceEditor.endpointRef.trim(), credentialRef: sourceEditor.credentialRef.trim(), enabled: true, health: 'unknown', priority: sources.length + 1, models: [{ id: `model-${now}`, name: modelName, displayName: modelName, contextWindow: 128000, maxOutputTokens: 8192, capabilities: ['structured_output', 'tool_calling'], enabled: true, health: 'unknown' }] }
    setSources([...sources, source]); setSourceEditor(null); notify('模型来源已加入前端草稿，尚未连接服务端。')
  }
  const probe = (source: GenerativeSourceDraft) => {
    setProbingSource(source.id)
    window.setTimeout(() => { setProbingSource(''); notify(`${source.name}：前端演示未发起真实探测。`, 'warning') }, 700)
  }
  return <div className="model-config-page">
    <nav className="model-config-tabs compact"><button className={tab === 'sources' ? 'active' : ''} onClick={() => setTab('sources')}><Server />模型来源<i>{sources.length}</i></button><button className={tab === 'models' ? 'active' : ''} onClick={() => setTab('models')}><Bot />全部模型<i>{allModels.length}</i></button></nav>
    {tab === 'sources' && <div className="model-config-panel"><ModelPanelHead title="模型来源" desc="统一维护可供各 Agent 使用的生成式模型渠道；凭据只保存引用。"><button className="btn primary" onClick={() => setSourceEditor({ name: '', providerType: 'openai_compatible', endpointRef: '', credentialRef: '', modelName: '' })}><Plus />添加来源</button></ModelPanelHead><div className="generative-source-grid">{sources.map(source => <article className={`generative-source-card ${source.enabled ? '' : 'disabled'}`} key={source.id}><header><div className="source-logo"><Server /></div><div><b>{source.name}</b><small>{providerLabel(source.providerType)}</small></div><Badge tone={source.enabled ? modelHealthTone(source.health) : 'gray'}>{source.enabled ? modelHealthLabel(source.health) : '已停用'}</Badge><label className="switch"><input type="checkbox" checked={source.enabled} onChange={event => updateSource(source.id, { enabled: event.target.checked })} aria-label={`启用 ${source.name}`} /><i /></label></header><div className="source-reference"><span>端点引用<b>{source.endpointRef}</b></span><span>凭据引用<b>{source.credentialRef.replace(/\/.+$/, '/••••••')}</b></span></div><div className="source-model-chips">{source.models.map(model => <span key={model.id}><i className={model.health} />{model.displayName}<small>{formatModelTokens(model.contextWindow)}</small></span>)}</div><footer><span>{source.models.length} 个模型</span><div><button className="btn ghost" disabled={probingSource === source.id} onClick={() => probe(source)}><Activity />{probingSource === source.id ? '探测中' : '连接探测'}</button><button className="icon-btn danger-text" onClick={() => deleteSource(source)} title="移除来源"><Trash2 /></button></div></footer></article>)}</div></div>}
    {tab === 'models' && <div className="model-config-panel"><ModelPanelHead title="全部模型" desc="跨来源聚合模型能力与健康状态，供 Agent 配置时统一选择。"><label className="model-search"><Search /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索模型或来源" /></label></ModelPanelHead><div className="generative-model-table"><div className="generative-model-row table-head"><span>模型</span><span>来源</span><span>上下文 / 输出</span><span>能力</span><span>健康状态</span><span>启用</span></div>{allModels.filter(model => `${model.displayName} ${model.name} ${model.source.name}`.toLowerCase().includes(query.trim().toLowerCase())).map(model => <div className="generative-model-row" key={model.id}><span><b>{model.displayName}</b><small>{model.name}</small></span><span><b>{model.source.name}</b><small>{model.source.enabled ? '来源已启用' : '来源已停用'}</small></span><span><b>{formatModelTokens(model.contextWindow)} / {formatModelTokens(model.maxOutputTokens)}</b><small>上下文 / 最大输出</small></span><span className="capability-list">{model.capabilities.map(capability => <i key={capability}>{capability === 'structured_output' ? '结构化' : capability === 'tool_calling' ? '工具调用' : '视觉'}</i>)}</span><span><Badge tone={modelHealthTone(model.health)}>{modelHealthLabel(model.health)}</Badge></span><span><label className="switch"><input type="checkbox" checked={model.enabled} onChange={event => updateSource(model.source.id, { models: model.source.models.map(item => item.id === model.id ? { ...item, enabled: event.target.checked } : item) })} aria-label={`启用 ${model.displayName}`} /><i /></label></span></div>)}</div></div>}
    {sourceEditor && <Modal title="添加生成式模型来源" onClose={() => setSourceEditor(null)}><div className="modal-form"><p>只保存部署侧端点和凭据引用，浏览器不接收 API Key 明文。</p><label>来源名称<input value={sourceEditor.name} onChange={event => setSourceEditor(current => current && { ...current, name: event.target.value })} placeholder="例如：OpenAI 灾备渠道" /></label><label>协议类型<select value={sourceEditor.providerType} onChange={event => setSourceEditor(current => current && { ...current, providerType: event.target.value as GenerativeSourceDraft['providerType'] })}><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option><option value="openai_compatible">OpenAI Compatible</option></select></label><label>端点引用<input value={sourceEditor.endpointRef} onChange={event => setSourceEditor(current => current && { ...current, endpointRef: event.target.value })} placeholder="OPENAI_BACKUP_BASE_URL" /></label><label>凭据引用<input value={sourceEditor.credentialRef} onChange={event => setSourceEditor(current => current && { ...current, credentialRef: event.target.value })} placeholder="secret://openai/backup" /></label><label>首个模型标识<input value={sourceEditor.modelName} onChange={event => setSourceEditor(current => current && { ...current, modelName: event.target.value })} placeholder="gpt-5.1-mini" /></label><div className="modal-actions"><button className="btn ghost" onClick={() => setSourceEditor(null)}>取消</button><button className="btn primary" onClick={saveSource}><Plus />加入草稿</button></div></div></Modal>}
  </div>
}
