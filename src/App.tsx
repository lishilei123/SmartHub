...
function ResourceMetadata({ resource, catalog }: { resource: AiResource; catalog: AiResourceCatalog | null }) {
  ...
  return <footer><span>{resource.source === 'builtin' ? '内置' : resource.source === 'local' ? '本地' : resource.source === 'http' ? 'HTTP' : `MCP · ${mcp?.name ?? '未解析'}`}</span><span>{resource.risk === 'read' ? '只读' : resource.risk === 'network_read' ? '网络只读' : resource.risk === 'code_execution' ? '代码执行' : '内部写入'}</span><span>{(resource.timeoutMs / 1000).toLocaleString()} 秒超时</span>{resource.sourcePath && <span title={resource.sourcePath}>{resource.sourcePath}</span>}{resource.endpoint && <span title={resource.endpoint}>{resource.endpoint}</span>}</footer>
}
