...
function RetrievalIndexConfig({notify}:{notify:(s:string)=>void}){
  ...
  return <div className="retrieval-config">
    ...
    <div className="retrieval-block"><div className="block-title"><div><b>Reranker 结果重排</b><small>对初步召回内容再次排序，提升最终引用质量</small></div><label className="switch"><input type="checkbox" checked={reranker} onChange={e=>setReranker(e.target.checked)}/><i/></label></div>{reranker&&<div className="reranker-fields"><label><span>重排方式</span><select><option>本地模型</option><option>远程 API</option></select></label><label><span>模型</span><select><option>BAAI / bge-reranker-v2-m3</option><option>bge-reranker-large</option></select></label></div>}</div>
