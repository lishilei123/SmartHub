...
function ModelManagementSettings({ draft, update, notify }: { draft: SettingsDraft; update: <K extends keyof SettingsDraft>(key: K, value: SettingsDraft[K]) => void; notify: Notify }) {
  ...
  const allModels = sources.flatMap(source => source.models.map(model => ({ ...model, source })))
  const filteredModels = allModels.filter(model => `${model.displayName} ${model.name} ${model.source.name}`.toLowerCase().includes(query.trim().toLowerCase()))
  const tabOrder: Array<typeof tab> = ['sources', 'models']
  ...
  const saveSource = () => {
    ...
    for (const model of normalizedModels) {
      ...
      else if (!Number.isInteger(model.contextWindow) || model.contextWindow <= 0 || !Number.isInteger(model.maxOutputTokens) || model.maxOutputTokens <= 0) modelErrors[model.id] = '上下文窗口和最大输出必须为正整数。'
      else if (model.maxOutputTokens > model.contextWindow) modelErrors[model.id] = '最大输出不能超过上下文窗口。'
  ...
  const testModel = (model: GenerativeModelDraft) => {
    setTestingModelId(model.id)
    window.setTimeout(() => { setTestingModelId(''); notify(`${model.displayName}：演示测试完成，未发起真实服务请求。`, 'warning') }, 500)
  }
  const selectTab = (nextTab: typeof tab) => setTab(nextTab)
  const handleTabKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>) => {
    const currentIndex = tabOrder.indexOf(tab)
    const nextIndex = event.key === 'ArrowRight' ? (currentIndex + 1) % tabOrder.length : event.key === 'ArrowLeft' ? (currentIndex - 1 + tabOrder.length) % tabOrder.length : event.key === 'Home' ? 0 : event.key === 'End' ? tabOrder.length - 1 : -1
    if (nextIndex < 0) return
    event.preventDefault()
    const nextTab = tabOrder[nextIndex]
    selectTab(nextTab)
    window.requestAnimationFrame(() => document.getElementById(`model-management-tab-${nextTab}`)?.focus())
  }
  return <div className="model-config-page">
    <nav className="model-config-tabs compact" role="tablist" aria-label="模型管理视图"><button id="model-management-tab-sources" type="button" role="tab" tabIndex={tab === 'sources' ? 0 : -1} aria-selected={tab === 'sources'} aria-controls="model-management-panel-sources" className={tab === 'sources' ? 'active' : ''} onClick={() => selectTab('sources')} onKeyDown={handleTabKeyDown}><Server />模型来源<i aria-hidden="true">{sources.length}</i><span className="visually-hidden">，{sources.length} 个来源</span></button><button id="model-management-tab-models" type="button" role="tab" tabIndex={tab === 'models' ? 0 : -1} aria-selected={tab === 'models'} aria-controls="model-management-panel-models" className={tab === 'models' ? 'active' : ''} onClick={() => selectTab('models')} onKeyDown={handleTabKeyDown}><Bot />全部模型<i aria-hidden="true">{allModels.length}</i><span className="visually-hidden">，{allModels.length} 个模型</span></button></nav>
    <div id="model-management-panel-sources" role="tabpanel" aria-labelledby="model-management-tab-sources" hidden={tab !== 'sources'}><div className="model-config-panel"><ModelPanelHead title="模型来源" desc="统一维护可供各 Agent 使用的生成式模型渠道；凭据只保存引用。"><button className="btn primary" onClick={() => openSourceEditor()}><Plus />添加来源</button></ModelPanelHead><div className="generative-source-grid">{sources.map(source => <article className={`generative-source-card ${source.enabled ? '' : 'disabled'}`} key={source.id} aria-label={`${source.name} 模型来源`}><header><div className="source-logo"><Server /></div><div><b>{source.name}</b><small>{providerLabel(source.providerType)}</small></div><Badge tone={source.enabled ? modelHealthTone(source.health) : 'gray'}>{source.enabled ? modelHealthLabel(source.health) : '已停用'}</Badge><label className="switch"><input type="checkbox" checked={source.enabled} onChange={event => updateSource(source.id, { enabled: event.target.checked })} aria-label={`启用 ${source.name}`} /><i /></label></header><div className="source-reference"><span>端点引用<b>{source.endpointRef}</b></span><span>凭据引用<b>{source.credentialRef.replace(/\/.+$/, '/••••••')}</b></span></div><div className="source-model-chips">{source.models.map(model => <span key={model.id}><i className={model.health} aria-hidden="true" />{model.displayName}<small>{formatModelTokens(model.contextWindow)}</small></span>)}</div><footer><span>{source.models.length} 个模型</span><div><button className="icon-btn" onClick={() => openSourceEditor(source)} title={`编辑来源 ${source.name}`} aria-label={`编辑来源 ${source.name}`}><Pencil /></button><button className="icon-btn danger-text" onClick={() => deleteSource(source)} title={`移除来源 ${source.name}`} aria-label={`移除来源 ${source.name}`}><Trash2 /></button></div></footer></article>)}</div></div></div>
    <div id="model-management-panel-models" role="tabpanel" aria-labelledby="model-management-tab-models" hidden={tab !== 'models'}><div className="model-config-panel"><ModelPanelHead title="全部模型" desc="跨来源聚合模型能力与健康状态，供 Agent 配置时统一选择。"><label className="model-search"><Search /><span className="visually-hidden">搜索模型或来源</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索模型或来源" aria-controls="generative-model-results" /></label></ModelPanelHead><p className="model-search-summary" role="status" aria-live="polite">显示 {filteredModels.length} 个模型</p>{filteredModels.length > 0 ? <div className="generative-model-table" id="generative-model-results" role="table" aria-label="全部模型"><div className="generative-model-row table-head" role="row"><span role="columnheader">模型</span><span role="columnheader">来源</span><span role="columnheader">上下文 / 输出</span><span role="columnheader">能力</span><span role="columnheader">健康状态</span><span role="columnheader">启用</span></div>{filteredModels.map(model => <div className="generative-model-row" role="row" key={model.id}><span role="cell" data-label="模型"><button className="model-test-button" disabled={testingModelId === model.id} onClick={() => testModel(model)} aria-label={`测试模型 ${model.displayName}`}><b>{testingModelId === model.id ? '测试中…' : model.displayName}</b><small>{model.name}</small></button></span><span role="cell" data-label="来源"><b>{model.source.name}</b><small>{model.source.enabled ? '来源已启用' : '来源已停用'}</small></span><span role="cell" data-label="上下文 / 输出"><b>{formatModelTokens(model.contextWindow)} / {formatModelTokens(model.maxOutputTokens)}</b><small>上下文 / 最大输出</small></span><span role="cell" data-label="能力" className="capability-list">{model.capabilities.map(capability => <i key={capability}>{capability === 'structured_output' ? '结构化' : capability === 'tool_calling' ? '工具调用' : '视觉'}</i>)}</span><span role="cell" data-label="健康状态"><Badge tone={modelHealthTone(model.health)}>{modelHealthLabel(model.health)}</Badge></span><span role="cell" data-label="启用"><label className="switch"><input type="checkbox" checked={model.enabled} onChange={event => updateSource(model.source.id, { models: model.source.models.map(item => item.id === model.id ? { ...item, enabled: event.target.checked } : item) })} aria-label={`启用 ${model.displayName}`} /><i /></label></span></div>)}</div> : <div className="model-search-empty" id="generative-model-results" role="status"><b>没有匹配的模型</b><span>请调整搜索条件，或返回模型来源查看配置。</span></div>}</div></div>
    {sourceEditor && <Modal title={sourceEditor.id ? '编辑生成式模型来源' : '添加生成式模型来源'} onClose={() => { setSourceEditor(null); setSourceEditorErrors({}) }}><div className="modal-form"><p>只保存部署侧端点和凭据引用，浏览器不接收 API Key 明文。</p><label>来源名称<input value={sourceEditor.name} onChange={event => setSourceEditor(current => current && { ...current, name: event.target.value })} placeholder="例如：OpenAI 灾备渠道" /></label><label>协议类型<select value={sourceEditor.providerType} onChange={event => setSourceEditor(current => current && { ...current, providerType: event.target.value as GenerativeSourceDraft['providerType'] })}><option value="openai">OpenAI</option><option value="anthropic">Anthropic</option><option value="openai_compatible">OpenAI Compatible</option></select></label><label>端点引用<input value={sourceEditor.endpointRef} onChange={event => setSourceEditor(current => current && { ...current, endpointRef: event.target.value })} placeholder="OPENAI_BACKUP_BASE_URL" /></label><label>凭据引用<input value={sourceEditor.credentialRef} onChange={event => setSourceEditor(current => current && { ...current, credentialRef: event.target.value })} placeholder="secret://openai/backup" /></label>{sourceEditorErrors.source && <span className="field-error">{sourceEditorErrors.source}</span>}<section className="generative-model-editor"><header><div><b>模型配置</b><small>模型标识用于调用，展示名称用于界面显示。</small></div><button type="button" className="btn ghost" onClick={() => setSourceEditor(current => current && { ...current, models: [...current.models, createGenerativeModel()] })}><Plus />添加模型</button></header><div>{sourceEditor.models.map((model, index) => <article key={model.id}><header><div><b>模型 {index + 1}</b>{model.id === draft.mainModel && <Badge tone="green">默认模型</Badge>}{draft.fallbackModelIds.includes(model.id) && <Badge tone="purple">回退模型</Badge>}</div><button type="button" className="icon-btn danger-text" disabled={sourceEditor.models.length === 1} title={sourceEditor.models.length === 1 ? '每个来源至少保留一个模型' : `移除模型 ${model.displayName || model.name || index + 1}`} aria-label={`移除模型 ${model.displayName || model.name || index + 1}`} onClick={() => removeEditorModel(model.id)}><Trash2 /></button></header><div className="generative-model-fields"><label>模型标识<input value={model.name} onChange={event => updateEditorModel(model.id, { name: event.target.value })} placeholder="gpt-5.1-mini" /></label><label>展示名称<input value={model.displayName} onChange={event => updateEditorModel(model.id, { displayName: event.target.value })} placeholder="GPT-5.1 Mini" /></label><label>上下文窗口<input type="number" min="1" step="1" value={model.contextWindow} onChange={event => updateEditorModel(model.id, { contextWindow: Number(event.target.value) })} /></label><label>最大输出<input type="number" min="1" step="1" value={model.maxOutputTokens} onChange={event => updateEditorModel(model.id, { maxOutputTokens: Number(event.target.value) })} /></label></div><div className="generative-model-options"><span>能力</span>{(['structured_output', 'tool_calling', 'vision'] as const).map(capability => <label key={capability}><input type="checkbox" checked={model.capabilities.includes(capability)} onChange={event => updateEditorModel(model.id, { capabilities: event.target.checked ? [...model.capabilities, capability] : model.capabilities.filter(item => item !== capability) })} />{capability === 'structured_output' ? '结构化输出' : capability === 'tool_calling' ? '工具调用' : '视觉'}</label>)}<label className="model-enabled"><input type="checkbox" checked={model.enabled} onChange={event => updateEditorModel(model.id, { enabled: event.target.checked })} />启用模型</label></div>{sourceEditorErrors.models?.[model.id] && <span className="field-error">{sourceEditorErrors.models[model.id]}</span>}</article>)}</div></section><div className="modal-actions"><button className="btn ghost" onClick={() => { setSourceEditor(null); setSourceEditorErrors({}) }}>取消</button><button className="btn primary" onClick={saveSource}>{sourceEditor.id ? <Check /> : <Plus />}{sourceEditor.id ? '保存修改' : '加入草稿'}</button></div></div></Modal>}
  </div>
}
