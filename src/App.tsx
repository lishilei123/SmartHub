...
function PromptAgentSettings({ draft, update, notify }: { draft: SettingsDraft; update: <K extends keyof SettingsDraft>(key: K, value: SettingsDraft[K]) => void; notify: Notify }) {
  ...
  return <div className="agent-settings-page">
    ...
    {tab === 'prompt' && <div className="model-config-panel agent-static-panel"><ModelPanelHead title="双 Agent Prompt" desc="提取与评审分别使用独立系统指令、任务模板和输出协议。"><Badge tone="green">Prompt V4.0</Badge></ModelPanelHead><div className="agent-static-grid"><article><span>RequirementPointExtractionAgent</span><b>直接理解正文并提交草稿</b><p>输出 requirementPoints、evidenceDrafts；Evidence 定位和 coverage 由服务端生成。</p></article><article><span>RequirementReviewAgent</span><b>只评审冻结提取结果</b><p>输出 summary、findings，协议不接受需求点、证据或覆盖范围改写。</p></article></div><p className="readonly-notice">当前为前端结构示例，不会发布或覆盖正式 Prompt。</p></div>}
    {tab === 'tools' && <div className="model-config-panel agent-static-panel"><ModelPanelHead title="工具与权限" desc="正文由服务端直接投递，工具只承担定向复核与结果提交。"><Badge tone="purple">工具集 V4.0</Badge></ModelPanelHead><div className="agent-static-grid"><article><span>提取工具</span><b>knowledge.search/read_chunk · evidence.validate_batch</b><p>可选定向复核固定 Chunk，并通过 requirement_points_submit_result 提交 v2 草稿。</p></article><article><span>评审工具</span><b>review_submit_result</b><p>只提交 Finding 与摘要，无法提交或改写需求点。</p></article></div></div>}
