...
function PromptAgentSettings({ draft, update, notify }: { draft: SettingsDraft; update: <K extends keyof SettingsDraft>(key: K, value: SettingsDraft[K]) => void; notify: Notify }) {
  ...
  return <div className="agent-settings-page">
    <section className="agent-config-header"><div className="agent-symbol"><BrainCircuit /></div><div><span>业务 Agent 流程</span><h3>RequirementPointExtractionAgent → RequirementReviewAgent</h3><p>需求点提取与需求评审 · 独立会话串联 · 配置草稿 V6</p></div><Badge tone="green">已启用</Badge></section>
    ...
    {tab === 'prompt' && <div className="model-config-panel agent-static-panel"><ModelPanelHead title="双 Agent Prompt" desc="提取与评审分别使用独立系统指令、任务模板和输出协议。"><Badge tone="green">Prompt V3.0</Badge></ModelPanelHead><div className="agent-static-grid"><article><span>RequirementPointExtractionAgent</span><b>只提取需求点与固定证据</b><p>输出 requirementPoints、evidence、coverage，不生成 Finding、评分或评审结论。</p></article><article><span>RequirementReviewAgent</span><b>只评审冻结提取结果</b><p>输出 summary、findings，协议不接受需求点、证据或覆盖范围改写。</p></article></div><p className="readonly-notice">当前为前端结构示例，不会发布或覆盖正式 Prompt。</p></div>}
    {tab === 'tools' && <div className="model-config-panel agent-static-panel"><ModelPanelHead title="工具与权限" desc="两个 Agent 的工具白名单分别固定，评审 Agent 不继承提取 Agent 工具。"><Badge tone="purple">工具集 V3.0</Badge></ModelPanelHead><div className="agent-static-grid"><article><span>提取工具</span><b>knowledge.* · evidence.validate</b><p>只读访问固定需求资产，并通过 requirement_points_submit_result 提交冻结结果。</p></article><article><span>评审工具</span><b>review_submit_result</b><p>只提交 Finding 与摘要，无法提交或改写需求点。</p></article></div></div>}
    {tab === 'versions' && <div className="model-config-panel"><ModelPanelHead title="Agent 配置版本" desc="模型选择、Prompt、工具与权限共同形成不可变版本。"><button className="btn ghost" onClick={() => notify('当前仅展示 Agent 版本前端原型。', 'warning')}><RefreshCw />刷新</button></ModelPanelHead><div className="model-version-list">{[{ version: 'V6', status: '当前生效', model: 'GPT-5.2', author: '李磊', time: '2026-07-22 16:40' }, { version: 'V4', status: '已停用', model: 'Claude Sonnet 4.5', author: '李磊', time: '2026-07-18 11:26' }, { version: 'V3', status: '已停用', model: 'SmartHub Review 72B', author: '系统管理员', time: '2026-07-12 09:15' }].map((item, index) => <div key={item.version}><span className="version-node"><i /></span><div><b>{item.version} · 双 Agent 流程</b><small>{item.time} · {item.author}</small></div><span><b>{item.model}</b><small>默认模型</small></span><Badge tone={index === 0 ? 'green' : 'gray'}>{item.status}</Badge><button className="btn ghost" onClick={() => notify(`${item.version} 为前端版本快照示例。`, 'warning')}>查看快照</button></div>)}</div></div>}
  </div>
}
