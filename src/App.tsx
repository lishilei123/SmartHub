...
function ResourceMetadata({ resource, catalog }: { resource: AiResource; catalog: AiResourceCatalog | null }) {
  ...
  if (resource.kind === 'skill') return <footer><span>{resource.package ? `ZIP · ${resource.package.fileCount} 个文件 · ${formatBytes(resource.package.unpackedBytes)}` : '手动入口'}</span><span>{resource.toolIds.length} 个外部依赖工具</span><span>运行开始时直接加载</span><span>{resource.tags.join(' · ') || '未设置标签'}</span><span title={resource.package?.contentSha256 ?? resource.entrypoint}>{resource.package ? `SHA-256 ${resource.package.contentSha256.slice(0, 12)}…` : resource.entrypoint}</span></footer>
...
function agentSkillSummary(skill: SkillResource) {
  return skill.toolIds.length ? `外部依赖：${skill.toolIds.join('、')}` : '发布后在运行开始时直接加载'
}
