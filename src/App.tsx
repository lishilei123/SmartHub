...
const agentConfigurationMetadata: Record<AgentConfigurationAgentKey, { label: string; identifier: string; sceneLabel: string; protocolLabel: string; publishTarget: string; runtimeToolIds: string[]; exactCapabilities: boolean }> = {
  planning: { label: 'PlanningAgent', identifier: 'PlanningAgent', sceneLabel: '测试策划', protocolLabel: 'Planning + Project Workspace v1', publishTarget: '新的需求分析与测试设计任务', runtimeToolIds: ['workspace.list_directory', 'workspace.find_files', 'workspace.grep_files', 'workspace.read_file', 'knowledge.search', 'knowledge.read_chunk', 'requirement-analysis.submit_result', 'requirement-repair.submit_result', 'requirement-release.submit_result', 'test_design_points.submit_result', 'test_design_cases.submit_result', 'test_design_repair.submit_result'], exactCapabilities: false },
...
function Dashboard({ navigate, projectVersion, onManageVersions }: { navigate: (page: PageKey, planningTab?: PlanningTab) => void; projectVersion: ProjectVersion | null; onManageVersions: () => void }) {
  return <div className="dashboard-grid"><section className="card span2 dashboard-notice"><Badge tone="violet"><Sparkles size={12} /> {projectVersion ? `当前版本 ${projectVersion.name}` : '尚未创建项目版本'}</Badge><h2>{projectVersion ? '当前项目空间已按版本隔离' : '先创建项目版本，再开始测试策划'}</h2><p>{projectVersion ? '需求发布、测试设计运行与 Planning Session 上下文都固定在当前版本。' : '平台固定服务 SmartHub 单项目，项目空间通过版本切换。'}</p><div><button className="btn primary" onClick={projectVersion ? () => navigate('planning', 'workflow') : onManageVersions}>{projectVersion ? '进入测试策划' : '新建项目版本'}</button></div></section><section className="card quick-card"><Sparkles /><h3>需求分析</h3><p>由同一个 PlanningAgent 在当前 Project Workspace 与 Planning Session 中完成。</p><button className="text-btn" onClick={() => navigate('planning', 'requirements')}>打开需求分析 <ChevronRight /></button></section><section className="card quick-card"><TestTube2 /><h3>测试设计</h3><p>Requirement Release 后由同一个 PlanningAgent 继续处理，正式门禁与 Coverage Audit 保持独立。</p><button className="text-btn" onClick={() => navigate('planning', 'test-design')}>打开测试设计 <ChevronRight /></button></section><section className="card quick-card"><Library /><h3>知识库</h3><p>知识库由平台单项目共享，不随项目版本复制。</p><button className="text-btn" onClick={() => navigate('documents')}>打开知识库 <ChevronRight /></button></section><section className="card quick-card"><Settings /><h3>系统设置</h3><p>模型与平台配置为全局资源，不参与版本隔离。</p><button className="text-btn" onClick={() => navigate('settings')}>打开系统设置 <ChevronRight /></button></section></div>
}
...
type GenerativeModelMetadata = Pick<GenerativeModelDraft, 'contextWindow' | 'maxOutputTokens' | 'capabilities'>

const genericModelMetadata = (): GenerativeModelMetadata => ({ contextWindow: 256_000, maxOutputTokens: 32_000, capabilities: ['structured_output', 'tool_calling'] })
const modelMetadataDefaults = (providerType: GenerativeSourceDraft['providerType'], name: string): GenerativeModelMetadata => {
  if (providerType === 'openai' && /^gpt-5\.6(?:-(?:sol|terra|luna))?$/iu.test(name.trim())) {
    return { contextWindow: 256_000, maxOutputTokens: 128_000, capabilities: ['structured_output', 'tool_calling', 'reasoning', 'vision'] }
  }
  return genericModelMetadata()
}
const sameModelMetadata = (model: GenerativeModelDraft, metadata: GenerativeModelMetadata) => model.contextWindow === metadata.contextWindow
  && model.maxOutputTokens === metadata.maxOutputTokens
  && [...model.capabilities].sort().join('\u0000') === [...metadata.capabilities].sort().join('\u0000')
const createGenerativeModel = (providerType: GenerativeSourceDraft['providerType'], discovered?: { name: string; displayName: string }): GenerativeModelDraft => ({
  ...
  ...modelMetadataDefaults(providerType, discovered?.name ?? ''),
  enabled: true,
  health: 'unknown',
})
...
function ModelManagementSettings({ draft, notify, onPersistSources, onHealthUpdated }: { draft: SettingsDraft; notify: Notify; onPersistSources: (sources: GenerativeSourceDraft[], removedModelIds: Set<string>) => Promise<GenerativeSourceDraft[]>; onHealthUpdated: (source: GenerativeSourceDraft) => void }) {
  ...
  const updateEditorModelName = (id: string, name: string) => setSourceEditor(current => {
    ...
    return {
      ...
      models: current.models.map(model => {
        ...
        const previousDefaults = modelMetadataDefaults(current.providerType, model.name)
        const nextDefaults = modelMetadataDefaults(current.providerType, name)
        return { ...model, name, ...(sameModelMetadata(model, previousDefaults) ? nextDefaults : {}) }
      }),
    }
  })
  ...
  const saveSource = async () => {
    ...
    for (const model of normalizedModels) {
      ...
      else if (!Number.isInteger(model.contextWindow) || model.contextWindow < 1_024) modelErrors[model.id] = '上下文窗口必须是不小于 1024 的整数。'
      else if (!Number.isInteger(model.maxOutputTokens) || model.maxOutputTokens < 1_024) modelErrors[model.id] = '最大输出 Token 必须是不小于 1024 的整数。'
      else if (model.maxOutputTokens > model.contextWindow) modelErrors[model.id] = '最大输出 Token 不能超过上下文窗口。'
...
function AiResourceManagement({ kind, notify }: { kind: AiResourceKind; notify: Notify }) {
  ...
  const description = kind === 'mcp' ? '注册远程 MCP 服务及其暴露的工具范围；当前只允许 HTTP/S 传输。' : kind === 'skill' ? '维护可复用专业方法与外部依赖，供 Agent 发布版本时绑定并在运行开始时直接加载。' : '统一治理可独立配置的内置、本地、HTTP 与 MCP 工具。'
...
function agentSkillSummary(skill: SkillResource) {
  ...
  return [skill.toolIds.length ? `外部依赖：${skill.toolIds.join('、')}` : '', capabilities, '发布后在运行开始时直接加载'].filter(Boolean).join(' · ')
}
