...
export const deleteProjectVersion = (id: string) => request<{ id: string; name: string; deletedBindings: number; deletedAnalysisRuns: number; deletedTestDesigns: number; deletedTestDesignRuns: number; deletedTestCaseSetVersions: number; workspaceCleanupTaskIds: string[]; deletedWorkspaceDirectories: number; deletedWorkspaceAssets: number }>(`/project-versions/${encodeURIComponent(id)}`, { method: 'DELETE' })
