    ...
    <div className="rav2-planning-layout">
      ...
      <main className="rav2-session-panel">
        ...
        <div className={`rav2-session-body ${view === 'conversation' ? 'conversation' : view === 'cases' ? 'cases' : 'detail'}`}>
          ...
          {view === 'cases' && <Suspense fallback={<div className="rav2-empty"><LoaderCircle className="rotating" /><h2>正在加载测试设计运行</h2><p>正在建立测试设计的正式上下文。</p></div>}>{linkedTestDesign ? <EmbeddedTestDesignPage key={`${selectedRun?.id}:${linkedTestDesign.designId}:${linkedTestDesign.runId}`} embedded projectVersion={projectVersion} onManageVersions={onManageVersions} notify={notify} linkedDesignId={linkedTestDesign.designId} linkedRunId={linkedTestDesign.runId} /> : <div className="rav2-empty"><LoaderCircle className="rotating" /><h2>正在创建测试设计运行</h2><p>需求分析完成后，服务端会绑定本次需求理解对应的测试设计运行。</p></div>}</Suspense>}
