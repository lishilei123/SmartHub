  ...
  return <div className="planning-page">
    ...
    {tab === 'workflow' && <section className="planning-workflow-view">
      ...
      {!props.projectVersion ? <div className="planning-empty">请先选择 ProjectVersion。</div> : !workflow ? <PlanningLoading label="正在读取 Planning Workflow…" /> : <>
        ...
        <div className="planning-stage-grid">{workflow.stageProfiles.map((stage, index) => <article key={stage.stage}><header><i>{index + 1}</i><span><b>{stageLabels[stage.stage] ?? stage.stage}</b><small>{stage.agentKey}</small></span>{stage.humanGate && <em>Human Gate</em>}</header><dl><div><dt>Allowed Skills</dt><dd>{stage.allowedSkillKeys.length ? stage.allowedSkillKeys.join(' · ') : '—'}</dd></div><div><dt>Allowed Tools</dt><dd>{stage.allowedToolIds.length ? stage.allowedToolIds.join(' · ') : '—'}</dd></div><div><dt>Submit Tool</dt><dd>{stage.submitToolId ?? '—'}</dd></div><div><dt>Result Schema</dt><dd>{stage.resultSchemaVersion ?? '—'}</dd></div><div><dt>Reviewer</dt><dd>{stage.reviewers.length ? stage.reviewers.join(' · ') : '—'}</dd></div></dl></article>)}</div>
