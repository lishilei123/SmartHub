...
function InputSelection(props: { loading:boolean;projectVersion:ProjectVersion;baselines:TechnicalBaseline[];assets:TechnicalSolutionAsset[];reviews:TechnicalReview[];selectedBaseline:string;setSelectedBaseline:(value:string)=>void;selectedAssets:string[];setSelectedAssets:(value:string[])=>void;reviewName:string;setReviewName:(value:string)=>void;agentReady:boolean;starting:boolean;start:()=>void;openHistory:(review:TechnicalReview)=>void;refresh:()=>void;uploadRef:React.RefObject<HTMLInputElement|null>;uploadState:'idle'|'running';uploadProgress:UploadProgress|null;uploadDirectory:string;apiState:'connecting'|'ready'|'offline';upload:(event:React.ChangeEvent<HTMLInputElement>)=>Promise<void>;onManageVersions:()=>void;onOpenKnowledge:()=>void }) {
  ...
  return <div className="tech-input-page"><header className="tech-setup-header"><div className="tech-setup-title-block"><div className="tech-setup-title-icon"><FileCode2 /></div><div><h2>技术方案评审工作台 · {props.projectVersion.name}</h2><p>上传并固定技术方案资料，对齐已完成的需求评审基线。</p></div></div><button className="btn ghost" onClick={props.refresh}><RefreshCw />刷新资料</button></header><div className="tech-setup-columns">
...
function Overview({result,run}:{result:TechnicalFormalResult;run:TechnicalRun}) { const stats=result.statistics; const extraction=run.executions?.technicalSolutionExtraction; const review=run.executions?.technicalSolutionReview; return <section className="tech-overview"><div className="stat-grid"><Stat label="需求总数" value={stats.totalRequirements}/><Stat label="已覆盖" value={stats.covered} tone="green"/><Stat label="部分覆盖" value={stats.partiallyCovered} tone="orange"/><Stat label="未覆盖" value={stats.notCovered} tone="red"/><Stat label="待确认" value={stats.needsConfirmation} tone="violet"/><Stat label="覆盖率" value={`${(stats.coverageRatio*100).toFixed(1)}%`} tone="blue"/></div><article className="summary-card"><h3>AI 技术方案摘要</h3><p>{result.summary.overview}</p><div><section><b>主要缺口</b>{result.summary.majorGaps.map(item=><span key={item}>{item}</span>)}</section><section><b>主要风险</b>{result.summary.majorRisks.map(item=><span key={item}>{item}</span>)}</section><section><b>建议处理顺序</b>{result.summary.recommendedOrder.map((item,index)=><span key={item}>{index+1}. {item}</span>)}</section></div></article><article className="snapshot-card"><h3>运行快照</h3><dl><dt>提取 Agent</dt><dd>{run.snapshot.agentDefinitions?.technicalSolutionExtraction.agentKey ?? run.snapshot.agentDefinition.agentKey} {run.snapshot.agentDefinitions?.technicalSolutionExtraction.version ?? run.snapshot.agentDefinition.version}</dd><dt>评审 Agent</dt><dd>{run.snapshot.agentDefinitions?.technicalSolutionReview.agentKey ?? '历史单阶段运行'} {run.snapshot.agentDefinitions?.technicalSolutionReview.version ?? ''}</dd><dt>模型</dt><dd>{run.modelLabel}</dd><dt>索引</dt><dd>{run.snapshot.indexVersionId}</dd><dt>输入投递</dt><dd>{run.snapshot.inputPlan.mode} · {run.snapshot.inputPlan.batches.length} 批 · {run.snapshot.inputPlan.estimatedInputTokens} Token</dd><dt>执行</dt><dd>提取 {extraction?.turns??0} 轮 / {extraction?.toolCalls??0} 次工具；评审 {review?.turns??run.execution?.turns??0} 轮 / {review?.toolCalls??run.execution?.toolCalls??0} 次工具</dd></dl></article></section> }
function RunPlaceholder({run}:{run:TechnicalRun}) {
  const phases = ['校验固定输入', '提取技术方案要点', '评审冻结方案要点', '发布正式结果']
  ...
  return <section className={`run-placeholder ${run.status}`}>
    <div className="run-placeholder-hero"><span className="run-state-icon">{active ? <RefreshCw className="spin" /> : failed ? <XCircle /> : <AlertTriangle />}</span><div><small>{run.status === 'queued' ? '等待执行' : run.status === 'running' ? '实时运行' : failed ? '运行失败' : '运行已结束'}</small><h3>{run.status === 'queued' ? '等待 Worker 领取评审任务' : run.status === 'running' ? run.step === 'reviewing_solution' ? 'TechnicalSolutionReviewAgent 正在评审冻结方案要点' : 'TechnicalSolutionExtractionAgent 正在提取技术方案要点' : failed ? '本次运行失败，未发布正式结果' : '本次运行未发布正式结果'}</h3><p>{run.error ?? '两个 Agent 使用独立会话；页面持续同步服务端阶段和进度，刷新或关闭浏览器不会中断本次评审。'}</p></div></div>
...
function RunContextPanel({run}:{run:TechnicalRun}) { return <div className="tech-run-context"><section><ShieldCheck /><span><b>本次上下文已固定</b><small>运行始终使用创建时的版本、正文 Hash、两个 Agent 与模型。</small></span></section><dl><div><dt>项目版本</dt><dd>{run.snapshot.projectVersionName}</dd></div><div><dt>需求点</dt><dd>{run.snapshot.requirementBaseline.requirementPoints.length} 个</dd></div><div><dt>技术方案</dt><dd>{run.snapshot.solutionInputs.length} 份</dd></div><div><dt>方案要点</dt><dd>{run.extractionResult?.solutionPoints.length ?? 0} 个</dd></div><div><dt>Agent</dt><dd>技术方案提取 + 技术方案评审</dd></div><div><dt>模型</dt><dd>{run.modelLabel}</dd></div><div><dt>活动索引</dt><dd title={run.snapshot.indexVersionId}>{run.snapshot.indexVersionId}</dd></div></dl><div className="tech-context-note"><Clock3 /><span><b>{stepLabel(run.step)}</b><small>提取结果冻结后才进入评审；完成后发布覆盖、Finding、风险和 Evidence。</small></span></div></div> }
...
function stepLabel(value:string){return({waiting_worker:'等待 Worker',validating_input:'校验输入',assembling_context:'组装正文',extracting_solution_points:'提取技术方案要点',reviewing_solution:'评审冻结方案要点',analyzing_solution:'历史单阶段分析',candidate_submitted:'结果已提交',resolving_evidence:'解析 Evidence',validating_result:'校验结果',publishing_result:'保存正式结果',succeeded:'已完成',failed:'失败',cancelled:'已取消'} as Record<string,string>)[value]??value}
function runPhase(value:string){return({waiting_worker:0,validating_input:0,assembling_context:0,extracting_solution_points:1,analyzing_solution:1,reviewing_solution:2,candidate_submitted:2,resolving_evidence:2,validating_result:2,publishing_result:3,succeeded:4} as Record<string,number>)[value]??0}
