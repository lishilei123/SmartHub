...
export class PiTestDesignRuntimeAdapter implements TestDesignAgentRuntime {
  ...
  async execute(input: Parameters<TestDesignAgentRuntime['execute']>[0], signal: AbortSignal) {
    ...
    const task = canonicalJson({ schemaVersion: 'test-design-agent-input/v1', stage: input.stage, runId: input.run.id, projectVersionId: input.run.projectVersionId, basisSnapshot: input.run.basisSnapshot, retrievalSnapshot: input.run.retrievalSnapshot, historicalSnapshot: input.run.historicalSnapshot, upstream: input.upstream })
