...
export function renderPlanningRequirementTask(snapshot: ReviewRunSnapshot) {
  ...
  return [
    ...
    [
      ...
      '4. Knowledge、测试最佳实践、额外边界、错误文案和覆盖深度不得 blocking；不要为普通风险创建非阻断 Clarification 或替代性的结构化中间产物。后续测试设计会从正式 Requirement、Workspace 和 Knowledge 直接重新探索。',
    ...
    [
      ...
      '本轮结果范围是需求理解与必要的 Clarification；后续测试设计直接使用正式 Requirement、Clarification、Workspace 与 Knowledge。',
