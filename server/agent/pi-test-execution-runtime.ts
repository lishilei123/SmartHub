...
function stageInstructions(
  ...
  if (stage === 'failure_diagnosis') return [
    ...
    'HTTP 404/405 本身不证明脚本错误。只按冻结 Verification Check 中明确要求的状态码判断预期，不以标题、步骤或偶然数字为准；可信固定契约明确证明路径或方法错误时才可归类 script_defect。请求符合契约时结合环境、数据及终态证据诊断，产品缺陷仍为待确认候选；缺少相关契约或独立根因证据时提交 unknown，不自动修复。',
    ...
    '若失败来自脚本自行增加、但冻结 Expected Result/Verification Check 未要求的提示文案、Toast、状态码或其他额外门槛，应归类 script_defect；assertion_mismatch 只用于受保护断言本身与冻结预期存在语义冲突。404/405 必须优先遵守上述可信契约规则，不能由额外状态码门槛直接归因或触发修复。',
  ...
  const implementation = [
    ...
    '每次 Service 重试都是新的提交校验 invocation；提交 API 脚本前必须在本次 invocation 真正读取当前冻结版本中相关接口文档、受管实现、适用当前环境的探索记录或固定 Knowledge Chunk。目录位置不决定可信；读取记录必须关联来源、内容 Hash、作用域及候选 Endpoint/Method。只搜索、历史记忆、无关文件或 Agent 自称已读取均不够；缺少证据应说明缺项并补读。',
    ...
    'API 异常场景不得把业务失败自行等同于 HTTP 4xx：只有冻结 Expected Result 明确要求 HTTP/状态码时，受保护断言才能锚定状态码或 4xx 范围。否则应把冻结的业务错误字段、成功标志或持久化不变量作为受保护断言；未明确预期的 404/405 仅作为非锚定防误报保护，失败后先核对可信契约，不能直接归因或自动修复，不能把不存在的路径、错误方法或“200 + 业务失败响应”误判。',
    '重复删除、幂等或恢复性 Case 若未冻结具体响应状态，应通过目标仍不存在、对照数据仍可读、后续列表/查询仍成功等持久化不变量证明结果；不得把 response.ok()、4xx 范围或任意单一状态码作为受保护业务断言。',
    'UI 拒绝类 Case 若冻结 Expected Result 未指定错误文案，不得额外要求某个 Toast、提示文本或正则必须可见；冻结 Verification Check 明确要求持久化时，须在刷新或回读后验证同一业务结果。只读或不存在资源的查询不因此强制写入。',
    ...
    '业务 URL 可作为输入数据、响应字段或断言值；实际请求仍受冻结 targets 的 Runner 网络治理。UI 导航允许直接 goto 和可证明实际调用的受管 helper/Page Object；未知动态分派、fixture 导航或无法证明的回读需补充受支持实现或人工处理，不得凭源码出现 goto 或方法名宣称验证完成。',
  ...
  if (stage === 'script_repair') implementation.push(
    ...
    '只有可信固定契约和执行证据明确证明辅助 API 路径或方法错误，才删除无效调用或改用已证明的接口验证同一受保护业务不变量；404/405 本身不证明路由无效。证据不足交由人工处理，不能为通过而移除必要调用。',
