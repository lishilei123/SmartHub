...
export const REQUIREMENT_POINT_EXTRACTION_AGENT_VERSION = '3.2.0'
...
逐份读取全部固定输入资产。knowledge_read_asset 的目录是分页的，只要 outlinePage.hasMore 为 true，就必须使用 outlineOffset 继续读取，直到 hasMore 为 false；随后必须逐个调用 knowledge_read_chunk 读取运行快照 extractionCoveragePlan 中每个未排除 Chunk。提交 coverage.assets 时只能把实际读取的 Chunk 写入 reviewedChunkIds；预先排除的 Chunk 必须按计划写入 skippedChunks，禁止自行跳过正文或编造覆盖。
...
每个需求点至少绑定一条有效固定证据。quote 必须先通过 knowledge_read_chunk 获取，再连续逐字复制到 evidence_validate；校验成功后使用工具返回的 locator。若 evidence_validate 返回 valid=false，必须先修正 quote、Chunk 或资产版本并重新校验；不得提交任何未通过校验、少于 4 个字符或包含省略号的摘录。若 knowledge_read_chunk 返回 replayed=true，表示重放本次运行已成功读取的固定结果；直接使用其内容，禁止再次以相同参数调用。
...
固定覆盖计划：{{coveragePlan}}。最小工具预算：{{minimumToolCalls}} 次调用。
逐份读完分页目录，并读取覆盖计划中每个未排除 Chunk；不要因单条证据或局部章节已完成就提前提交。按可独立实现、测试或验收的粒度输出原子需求点、固定证据及结构化覆盖记录。不得生成 Finding、评分或评审结论。只通过 requirement_points_submit_result 提交结果。`
...
export function renderRequirementTask(snapshot: ReviewRunSnapshot, fixedExtraction?: CandidateRequirementPointExtraction) {
  ...
  return template
    ...
    .replace('{{coveragePlan}}', JSON.stringify(snapshot.extractionCoveragePlan))
    .replace('{{minimumToolCalls}}', String(snapshot.extractionToolBudget.minimumToolCalls))
