...
export class RequirementPointExtractionValidator {
  ...
  async validate(input: CandidateRequirementPointExtraction, snapshot: ReviewRunSnapshot): Promise<ValidationReport> {
...
export class RequirementReviewValidator {
  async validate(input: CandidateRequirementReview, extraction: CandidateRequirementPointExtraction, snapshot: ReviewRunSnapshot): Promise<ValidationReport> {
    ...
    const evidenceIds = new Set(extraction.evidence.map(evidence => evidence.clientEvidenceId))
    const pointEvidenceRefs = new Map(extraction.requirementPoints.map(point => [point.clientRequirementPointId, new Set(point.evidenceRefs)]))
    ...
    input.findings.forEach((finding, position) => {
      ...
      if (!isStrings(finding.evidenceRefs) || findingEvidenceRefs.some(reference => !evidenceIds.has(reference))) issues.push(issue(`${path}.evidenceRefs`, '包含无效固定证据引用'))
