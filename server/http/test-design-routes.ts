...
export async function routeTestDesign(request: IncomingMessage, response: ServerResponse, context: { method: string; url: URL; principal: Principal; controls: AccessControl; service: TestDesignService; store: StateStore }) {
  ...
  const batchAcceptReuse = /^\/api\/project-versions\/([^/]+)\/test-designs\/([^/]+)\/runs\/([^/]+)\/case-change-proposals\/batch-accept-unchanged-reuse$/.exec(url.pathname)
  if (batchAcceptReuse && method === 'POST') { await authorize(batchAcceptReuse[1], 'test-design:review'); return send(response, 201, await service.batchAcceptUnchangedReuseProposals(batchAcceptReuse[1], batchAcceptReuse[2], batchAcceptReuse[3], await json(request) as never, principal)) }
