...
export async function start(port = Number(process.env.PORT ?? 8787)) {
  ...
  const interruptedTasks = await service.recoverInterruptedTasks()
  ...
  return new Promise<typeof server>(resolvePromise => server.listen(port, '127.0.0.1', () => {
    interruptedTasks.forEach(taskId => scheduleTask(taskId, 0))
...
async function route(request: IncomingMessage, response: ServerResponse) {
  ...
  if (method === 'POST' && retry) { const retried = await service.retry(retry[1]); scheduleTask(retried.id); return send(response, 202, retried) }
  ...
  if (method === 'DELETE' && asset) { const result = await service.deleteAsset(asset[1]); scheduleTask(result.task.id); return send(response, 202, result) }
  ...
  if (method === 'POST' && rebuild) { const body = await json(request); if (body.outcome) return send(response, 202, await service.rebuild(rebuild[1], body.outcome as 'success' | 'failure' | 'cancel')); const task = await service.queueRebuild(rebuild[1]); if (task.status === 'queued') scheduleTask(task.id, 100); return send(response, 202, { task }) }
...
function scheduleTask(taskId: string, delay = 0) { setTimeout(() => { void service.processTask(taskId).catch(error => console.error(`知识库任务 ${taskId} 调度失败：`, error instanceof Error ? error.message : error)) }, delay) }
