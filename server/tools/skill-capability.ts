...
export class SkillCapabilityRuntime {
  ...
  register(definition: AgentDefinitionVersion, registry: ToolRegistry) {
    if (definition.toolIds.includes(SKILL_EXECUTE_SCRIPT_TOOL_ID)) this.registerScriptTool(registry)
    if (definition.toolIds.includes(SKILL_HTTP_REQUEST_TOOL_ID)) this.registerNetworkTool(registry)
  }
