...
export class ReviewQaService {
  ...
  async ask(runId: string, request: ReviewQuestionRequest, signal = new AbortController().signal) {
    ...
    const version = required(state.versions.find(item => item.id === run.assetVersionId && item.status === 'ready'), '评审绑定的固定资产版本不可用')
    ...
    const quote = normalizeQuote(request.quote, run.assetVersionId, version.content, run.result)
    const estimatedTokens = Math.ceil((version.content.length + JSON.stringify(run.result).length + question.length) / 4) + 2_000
...
function normalizeQuote(value: ReviewQuestionQuote | undefined, assetVersionId: string, content: string, result: NonNullable<ReviewRun['result']>) {
  if (!value) return undefined
  const text = String(value.text ?? '').trim().slice(0, 1_000)
  if (!text) return undefined
  if (value.assetVersionId !== assetVersionId) throw new Error('引用内容不属于本次评审的固定资产版本')
  ...
  return { ...value, text, assetVersionId, heading: String(value.heading ?? '').trim().slice(0, 300) || '固定需求原文' }
}
