...
export function auditTestDesignCoverage(input: {
  ...
  const caseSetSha256 = canonicalSha256(current.map(item => ({ caseId: item.caseId, revision: item.revision, contentSha256: item.contentSha256 })).sort((left, right) => left.caseId.localeCompare(right.caseId)))
  ...
  for (const item of current) {
    ...
    for (const ref of content.requirementRefs) {
      ...
      covered.add(ref)
      relations.push({ basisRef: ref, requirementId: ref, caseId: item.caseId, status: 'covered', reason: 'Effective Case Set 通过 requirementRefs 显式引用 coverageTarget Requirement' })
    }
