...
import type {
  CaseChangeDecision, CaseChangeProposal, CreateTestDesignInput, CoverageAudit, HistoricalCaseSnapshot, ImpactedRegressionReference, LibraryTestCase, LibraryTestCaseRevision, RetrievalSnapshot, SmokeCandidateRelation, TestCase,
  ...
  TestDesignNodeKey, TestDesignRunAgentConfigurationSnapshot, TestDesignState, TestDesignWorkflowRun, TestExecutionHandoff, TestExecutionMethod, TestPointNodeRevision,
  ...
  async createLibraryHandoff(projectVersionId: string, libraryVersionId: string, input: { mode: 'smoke' | 'regression' | 'full' | 'custom'; suiteVersionId?: string; impactedCaseIds?: string[]; expectedLibrarySha256: string }, principal: Principal) {
    return this.store.transaction(state => {
      ...
      const members = selections.map((selection, ordinal) => {
        const testCase = required(aggregate.libraryCases.find(item => item.id === selection.caseId && item.projectId === projectVersion.projectId), 'LIBRARY_TEST_CASE_NOT_FOUND', '正式测试用例不存在'); const revision = required(testCase.revisions.find(item => item.revision === selection.revision), 'LIBRARY_TEST_CASE_REVISION_NOT_FOUND', '正式用例 Revision 不存在'); const method = selection.executionMethod ?? executionMethodForContent(revision.content); if (selection.executionMethod && selection.executionMethod !== executionMethodForContent(revision.content)) throw new TestDesignError('TEST_SUITE_EXECUTION_METHOD_INVALID', '套件执行方式与用例 executionSpec 不一致', 422); const reason = cleanRequired(selection.reason, '选择原因', 2_000); return { stage: input.mode, ordinal, sourceVersionId: suite?.id ?? libraryVersion.id, caseId: testCase.id, revision: revision.revision, method, reason, dedupKey: `${testCase.id}:${revision.revision}:${method}`, dimension: revision.content.dimension, executionSpec: required(revision.content.executionSpec, 'TEST_CASE_EXECUTION_SPEC_INVALID', '正式用例缺少 executionSpec'), selectionReason: reason, contentSha256: revision.contentSha256 }
      })
...
function buildHistoricalSnapshot(state: DatabaseState, design: TestDesign, createdAt: string): HistoricalCaseSnapshot {
  ...
  if (selection.mode === 'suite_version') {
    ...
    const libraryIds = [...new Set(suite.members.map(item => item.testCaseLibraryVersionId).filter((item): item is string => Boolean(item)))]
    if (libraryIds.length !== 1) throw new TestDesignError('TEST_DESIGN_HISTORICAL_SOURCE_INVALID', '历史测试套件没有固定唯一用例库版本', 422)
    libraryVersion = aggregate.libraryVersions.find(item => item.id === libraryIds[0] && item.projectId === design.projectId)
  ...
  const base = { schemaVersion: 'historical-case-snapshot/v1' as const, items, createdAt }
  return { ...base, snapshotSha256: canonicalSha256(base) }
}
...
function createLibraryRevision(revision: number, content: TestCaseContent, actorId: string, changeReason: string, sourceRunId?: string, sourceProposalId?: string): LibraryTestCaseRevision { return { revision, content: structuredClone(content), contentSha256: canonicalSha256(content), semanticSha256: canonicalSha256({ ...content, tags: [...content.tags].sort() }), ...(sourceRunId ? { sourceRunId } : {}), ...(sourceProposalId ? { sourceProposalId } : {}), changeReason, createdBy: actorId, createdAt: now() } }
...
function suiteDraftInput(raw: unknown): { suiteKey: string; suiteType: 'smoke' | 'regression' | 'custom'; name: string; members: Array<{ testCaseLibraryVersionId: string; caseId: string; executionMethod: TestExecutionMethod; reason: string }> } { if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new TestDesignError('TEST_SUITE_DRAFT_INVALID', '套件草稿必须是对象', 422); const input = raw as Record<string, unknown>; const suiteType = String(input.suiteType); if (!['smoke', 'regression', 'custom'].includes(suiteType)) throw new TestDesignError('TEST_SUITE_DRAFT_INVALID', 'suiteType 必须为 smoke、regression 或 custom', 422); if (!Array.isArray(input.members) || input.members.length > 10_000) throw new TestDesignError('TEST_SUITE_DRAFT_INVALID', 'members 必须是最多 10000 项的数组', 422); return { suiteKey: cleanRequired(input.suiteKey, 'suiteKey', 200), suiteType: suiteType as 'smoke' | 'regression' | 'custom', name: cleanRequired(input.name, '套件名称', 200), members: input.members.map((candidate, index) => { if (!candidate || typeof candidate !== 'object' || Array.isArray(candidate)) throw new TestDesignError('TEST_SUITE_DRAFT_INVALID', `members[${index}] 必须是对象`, 422); const item = candidate as Record<string, unknown>; const executionMethod = String(item.executionMethod) as TestExecutionMethod; if (!['ui', 'api', 'performance_tool', 'long_running', 'environment_matrix'].includes(executionMethod)) throw new TestDesignError('TEST_SUITE_EXECUTION_METHOD_INVALID', `members[${index}].executionMethod 无效`, 422); return { testCaseLibraryVersionId: cleanRequired(item.testCaseLibraryVersionId, 'testCaseLibraryVersionId', 500), caseId: cleanRequired(item.caseId, 'caseId', 500), executionMethod, reason: cleanRequired(item.reason, '选择原因', 2_000) } }) } }
function validateSuiteMembers(aggregate: TestDesignState, projectId: string, values: ReturnType<typeof suiteDraftInput>['members']): TestSuiteVersionMember[] { const seen = new Set<string>(); return values.map((value, ordinal) => { if (seen.has(value.caseId)) throw new TestDesignError('TEST_SUITE_MEMBER_DUPLICATE', '套件成员不能重复', 422); seen.add(value.caseId); const version = required(aggregate.libraryVersions.find(item => item.id === value.testCaseLibraryVersionId && item.projectId === projectId), 'TEST_CASE_LIBRARY_VERSION_NOT_FOUND', '套件引用的用例库版本不存在'); const member = required(version.members.find(item => item.caseId === value.caseId), 'TEST_CASE_LIBRARY_MEMBER_NOT_FOUND', '套件成员不属于引用的用例库版本'); const testCase = required(aggregate.libraryCases.find(item => item.id === value.caseId && item.projectId === projectId), 'LIBRARY_TEST_CASE_NOT_FOUND', '正式用例不存在'); const revision = required(testCase.revisions.find(item => item.revision === member.revision), 'LIBRARY_TEST_CASE_REVISION_NOT_FOUND', '正式用例 Revision 不存在'); if (executionMethodForContent(revision.content) !== value.executionMethod) throw new TestDesignError('TEST_SUITE_EXECUTION_METHOD_INVALID', '套件执行方式与用例 executionSpec 不一致', 422); return { testCaseLibraryVersionId: version.id, caseId: testCase.id, revision: revision.revision, executionMethods: value.executionMethod === 'ui' || value.executionMethod === 'api' ? [value.executionMethod] : [], executionMethod: value.executionMethod, ordinal, reason: value.reason } }) }
...
function designState(state: DatabaseState): TestDesignState { return state.testDesignState ??= emptyTestDesignState() }
function readDesignState(state: DatabaseState): TestDesignState { return state.testDesignState ?? emptyTestDesignState() }
function emptyTestDesignState(): TestDesignState { return { architectureVersion: 'single-agent-skills/v1', designs: [], runs: [], caseSetVersions: [], libraryCases: [], libraryVersions: [], suiteDrafts: [], suiteVersions: [], executionHandoffs: [] } }
