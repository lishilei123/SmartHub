...
function reviewBasisItems(state: DatabaseState, reviewRunId: string, technicalRunId: string) {
  const review = required(state.reviewRuns.find(item => item.id === reviewRunId && item.status === 'succeeded' && item.result), 'TEST_DESIGN_REVIEW_CHAIN_INVALID', '需求评审基线无效')
  ...
  const reviewResult = required(review.result, 'TEST_DESIGN_REVIEW_CHAIN_INVALID', '需求评审结果不存在')
  const requirements = reviewResult.requirementPoints.map((point, index) => ({
