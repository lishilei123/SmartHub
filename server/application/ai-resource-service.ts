...
const builtInSkills: SkillResource[] = [
  ...
  builtInSkill('test-script-generation', '测试脚本生成', '结合 ProjectVersion Exploration Context、固定 Knowledge、Execution Workspace 与受控 Browser Tools 实现 Playwright UI 或 API Case。', '1.10.0', 'server/skills/test-script-generation/SKILL.md', ['browser.snapshot', 'browser.click', 'browser.fill', 'browser.get_locator', 'browser.requests', 'browser.request_detail', 'browser.screenshot'], ['测试执行', 'Playwright', '脚本生成']),
  builtInSkill('failure-analysis', '执行失败分析', '根据终态 Attempt 与不可变证据提交最小失败分类。', '1.8.0', 'server/skills/failure-analysis/SKILL.md', [], ['测试执行', '诊断', '失败分析']),
  builtInSkill('script-repair', '测试脚本修复', '在受保护断言语义不变的前提下，结合失败证据与受控 Browser Tools 提交最小 Playwright 源码修复。', '1.9.0', 'server/skills/script-repair/SKILL.md', ['browser.snapshot', 'browser.click', 'browser.fill', 'browser.get_locator', 'browser.requests', 'browser.request_detail', 'browser.screenshot'], ['测试执行', 'Playwright', '脚本修复']),
]
