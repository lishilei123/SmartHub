...
import type { TechnicalSolutionFindingAction, TechnicalSolutionFormalResult, TechnicalSolutionReview, TechnicalSolutionReviewRun, TechnicalSolutionRunSnapshot } from '../domain/technical-solution-types.js'
...
export class TechnicalSolutionReviewService {
  ...
  async inputCandidates(projectVersionId: string) {
    ...
    const configuration = this.definitions.resolveActive ? await this.definitions.resolveActive('technical-solution-analysis') : null
    return { projectVersion: { id: projectVersion.id, name: projectVersion.name, status: projectVersion.status }, baselines, solutionAssets, agentConfiguration: configuration ? { id: configuration.id, version: configuration.version, contentSha256: configuration.contentSha256, toolIds: configuration.agentDefinition.toolIds, primaryModel: configuration.routing.primaryModel } : null }
  }
  ...
  async createRun(projectVersionId: string, technicalReviewId: string) {
    ...
    const configuration = this.definitions.resolveActive ? await this.definitions.resolveActive('technical-solution-analysis') : null
    if (!configuration) throw new Error('TECH_AGENT_CONFIGURATION_UNAVAILABLE: 请先发布技术方案分析 Agent 配置')
    const route = selectModelRoute(state, configuration)
    const selection = route[0]
    const definition = configuration.agentDefinition
    if (definition.agentKey !== 'technical-solution-analysis' || definition.resultSchemaVersion !== 'technical-solution-review/v1') throw new Error('TECH_AGENT_CONFIGURATION_UNAVAILABLE: 已发布配置协议不匹配')
    const effectiveMaxOutput = Math.min(selection.model.maxOutputTokens, configuration.routing.maxOutputTokens)
    const inputPlan = buildTechnicalSolutionInputPlan({ assets: solutionAssets.map((asset, index) => ({ asset, version: solutionVersions[index] })), definition, contextWindow: selection.model.contextWindow, maxOutputTokens: effectiveMaxOutput })
    ...
    const snapshot: TechnicalSolutionRunSnapshot = {
      schemaVersion: 'technical-solution-run-snapshot/v1', runId, technicalReviewId, projectId: project.id, projectName: project.name, projectVersionId, projectVersionName: projectVersion.name, knowledgeBaseId: knowledgeBase.id,
      ...
      modelRoute: route.map(item => ({ sourceId: item.source.id, providerType: item.source.providerType, modelId: item.model.id, modelName: item.model.name, contextWindow: item.model.contextWindow, maxOutputTokens: Math.min(item.model.maxOutputTokens, configuration.routing.maxOutputTokens), supportsReasoning: item.model.capabilities.includes('reasoning') })),
      agentConfigurationRef: { id: configuration.id, version: configuration.version, contentSha256: configuration.contentSha256 }, agentDefinition: structuredClone(definition), inputPlan, createdAt: now,
    }
    const snapshotSha256 = sha256(stableStringify(snapshot))
    const queued = Boolean(this.store.enqueueTechnicalSolutionJob)
    const run: TechnicalSolutionReviewRun = { id: runId, technicalReviewId, projectVersionId, sourceReviewRunId: review.sourceReviewRunId, status: queued ? 'queued' : 'running', step: queued ? 'waiting_worker' : 'validating_input', progress: queued ? 1 : 5, snapshotSha256, snapshot, modelLabel: `${selection.source.name} · ${selection.model.displayName}`, createdAt: now, ...(queued ? {} : { startedAt: now }) }
  ...
  async processPreparedRun(runId: string, lease?: TaskLease, signal = new AbortController().signal, infrastructureAttempt = 1) {
    ...
    const state = await this.store.snapshot()
    const configuration = required(await this.definitions.resolveActive?.('technical-solution-analysis'), 'TECH_AGENT_CONFIGURATION_UNAVAILABLE')
    const route = stored.snapshot.modelRoute?.length ? stored.snapshot.modelRoute : [stored.snapshot.modelRef]
    const modelRef = route[Math.min(Math.max(1, infrastructureAttempt) - 1, route.length - 1)]
    const selection = selectSpecificModel(state, modelRef.sourceId, modelRef.modelId)
    const connection = { ...modelRef, baseUrl: selection.source.baseUrl, apiKey: selection.source.apiKey, temperature: configuration.routing.temperature, requestTimeoutMs: configuration.routing.requestTimeoutSeconds * 1_000, retryCount: configuration.routing.retryCount }
    const attemptId = `technical_attempt_${Math.max(1, infrastructureAttempt)}`
    const attemptStartedAt = new Date().toISOString()
    await this.runTransaction(runId, lease, draft => { const run = required(draft.technicalSolutionRuns.find(item => item.id === runId), 'TECH_RUN_NOT_FOUND'); const attempts = run.modelRouteAttempts ?? []; const current = { id: attemptId, attempt: Math.max(1, infrastructureAttempt), sourceId: modelRef.sourceId, modelId: modelRef.modelId, modelLabel: `${selection.source.name} · ${selection.model.displayName}`, status: 'running' as const, startedAt: attemptStartedAt }; const previous = attempts.findIndex(item => item.id === attemptId); if (previous >= 0) attempts[previous] = current; else attempts.push(current); const degradations = run.degradations ?? []; if ((modelRef.sourceId !== route[0].sourceId || modelRef.modelId !== route[0].modelId) && !degradations.some(item => item.toSourceId === modelRef.sourceId && item.toModelId === modelRef.modelId)) degradations.push({ fromSourceId: route[0].sourceId, fromModelId: route[0].modelId, toSourceId: modelRef.sourceId, toModelId: modelRef.modelId, reason: 'Provider 暂时错误后按已发布路由切换候选模型', occurredAt: attemptStartedAt }); Object.assign(run, { status: 'running', step: 'analyzing_solution', progress: 25, startedAt: run.startedAt ?? attemptStartedAt, modelLabel: current.modelLabel, modelRouteAttempts: attempts, degradations, error: undefined, errorCode: undefined }) })
    const events: AgentExecutionEvent[] = []
    try {
      const output = await this.runtime.execute({ snapshot: stored.snapshot, model: connection, requirementInputPlan: stored.snapshot.inputPlan, onEvent: async event => { events.push(event); if (checkpoint(event)) await this.saveProgress(runId, lease, events, progressForEvent(event)) } }, signal)
      if (!output.inputDeliveryManifest?.finalMergeCompleted || output.inputDeliveryManifest.entries.length !== stored.snapshot.inputPlan.batches.length) throw new Error('TECH_INPUT_DELIVERY_INCOMPLETE: 固定正文投递证明不完整')
      ...
      await this.runTransaction(runId, lease, draft => {
        ...
        Object.assign(run, { status: 'succeeded', step: 'succeeded', progress: 100, finishedAt, result: structuredClone(result), inputDeliveryManifest: output.inputDeliveryManifest, execution: executionRecord(output), events: output.events, error: undefined, errorCode: undefined })
      })
      return await this.loadRun(runId)
    } catch (error) {
      const message = sanitize(error)
      await this.runTransaction(runId, lease, draft => { const run = required(draft.technicalSolutionRuns.find(item => item.id === runId), 'TECH_RUN_NOT_FOUND'); const finishedAt = new Date().toISOString(); const attempt = run.modelRouteAttempts?.find(item => item.id === attemptId); if (attempt) Object.assign(attempt, { status: signal.aborted ? 'cancelled' : 'failed', finishedAt, error: message }); if (run.status !== 'cancelled') Object.assign(run, { status: signal.aborted ? 'cancelled' : 'failed', step: signal.aborted ? 'cancelled' : 'failed', finishedAt, errorCode: errorCode(message), error: message, events, execution: { agentKey: 'technical-solution-analysis', turns: maxTurn(events), toolCalls: toolCalls(events), toolErrors: toolErrors(events), events } }) }).catch(() => undefined)
      throw error
    }
  }
  ...
  async exportMarkdown(projectVersionId: string, technicalReviewId: string, runId: string) {
    ...
    lines.push('## 6. 主要风险', '', ...result.risks.map(item => `- ${safe(item.description)}；影响：${safe(item.impact)}；缓解：${safe(item.mitigation)}`), '', '## 7. 待确认问题', '', ...result.questions.map(item => `- ${safe(item.question)}：${safe(item.reason)}`), '', '## 8. Evidence', '', ...result.evidence.map(item => `- ${item.id} · ${item.sourceKind} · ${item.assetVersionId} · ${safe(item.headingPath.join(' / '))} · “${safe(item.quote)}”`), '', '## 9. 人工处置状态', '', ...actions.map(item => `- ${item.findingId} · ${item.fromState} → ${item.toState} · ${safe(item.actorDisplayName)} · ${safe(item.comment ?? '')}`), '', '## 10. 运行快照摘要', '', `- Agent：${run.snapshot.agentDefinition.agentKey} ${run.snapshot.agentDefinition.version}`, `- Prompt Hash：${run.snapshot.agentDefinition.promptRef.contentSha256}`, `- Toolset Hash：${run.snapshot.agentDefinition.toolsetContentSha256}`, `- IndexVersion：${run.snapshot.indexVersionId}`, `- 输入模式：${run.snapshot.inputPlan.mode}`, `- Snapshot Hash：${run.snapshotSha256}`, '', '## 11. 限制与降级', '', '- 本报告只评审固定需求基线与技术方案资料，不包含 Git、代码、部署或测试执行。')
    return lines.join('\n')
  }

  private async saveProgress(runId: string, lease: TaskLease | undefined, events: AgentExecutionEvent[], progress: number) {
    await this.runTransaction(runId, lease, state => { const run = required(state.technicalSolutionRuns.find(item => item.id === runId), 'TECH_RUN_NOT_FOUND'); run.progress = Math.max(run.progress, progress); run.step = progress >= 82 ? 'resolving_evidence' : 'analyzing_solution'; run.events = structuredClone(events); run.execution = { agentKey: 'technical-solution-analysis', turns: maxTurn(events), toolCalls: toolCalls(events), toolErrors: toolErrors(events), events: structuredClone(events) } })
  }
...
function executionRecord(output: AgentExecutionOutput) { return { agentKey: 'technical-solution-analysis' as const, turns: output.turns, toolCalls: output.toolCalls, toolErrors: output.toolErrors, framework: output.framework, events: output.events } }
function presentRunSummary(run: TechnicalSolutionReviewRun) { return { id: run.id, runId: run.id, technicalReviewId: run.technicalReviewId, projectVersionId: run.projectVersionId, sourceReviewRunId: run.sourceReviewRunId, status: run.status, step: run.step, progress: run.progress, modelLabel: run.modelLabel, modelRouteAttempts: run.modelRouteAttempts, degradations: run.degradations, createdAt: run.createdAt, startedAt: run.startedAt, finishedAt: run.finishedAt, errorCode: run.errorCode, error: run.error, queue: run.queue, snapshot: redactSnapshot(run.snapshot), summary: run.result?.summary, statistics: run.result?.statistics } }
function presentRun(run: TechnicalSolutionReviewRun) { return { ...presentRunSummary(run), ...(run.status === 'succeeded' ? { result: run.result, inputDeliveryManifest: run.inputDeliveryManifest, execution: run.execution } : { execution: run.execution }), events: run.events } }
function redactSnapshot(snapshot: TechnicalSolutionRunSnapshot) { return { ...snapshot, agentDefinition: { ...snapshot.agentDefinition, systemPrompt: undefined, taskTemplate: undefined }, inputPlan: { ...snapshot.inputPlan, batches: snapshot.inputPlan.batches.map(batch => ({ ...batch, content: undefined })) } } }
...
function progressForEvent(event: AgentExecutionEvent) { return event.type === 'tool_execution_end' && event.toolId === 'technical_solution_review.submit_result' ? 82 : event.type === 'input_final_merge_started' ? 65 : event.type === 'input_batch_delivered' ? 35 : 50 }
