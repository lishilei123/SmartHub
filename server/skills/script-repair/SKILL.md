...
4. Repair the persistent Execution Workspace entry and any directly related Page Object, helper, fixture or API client only when the diagnosis supports an automation defect. Keep the protected entry assertion semantics unchanged. Do not add shell commands, dependencies, dynamic imports, Node runtime access, arbitrary network access, database calls, or Runner instructions.
