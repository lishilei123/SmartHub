...
3. Use temporary case `ref` values. Submit each `cases[]` item as a flat `test-case/v2` object: `ref`, `schemaVersion`, `title`, and `executionSpec` are sibling fields. Never use a `{ ref, content: {...} }` wrapper. Dependencies and `dataRequirements[].caseRefs` must use those refs; leave `dataRequirementIds` empty because the server owns formal relationships.
