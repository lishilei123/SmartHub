...
- Content: every node must have a clear objective, dimension, priority, applicability, entry methods, oracle, data conditions, risks, assumptions, and at least one valid requirement basis ref. For a published Requirement, use its `requirements.json` `clientRequirementPointId` (for example `RP-001`); the Service resolves it only within this Run's frozen Requirement snapshot and records the canonical basis ID. Historical refs are optional supporting context.
