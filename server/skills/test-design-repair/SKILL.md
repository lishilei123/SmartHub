...
2. Submit only `test-design-repair/v3`: `schemaVersion`, `baseCandidateSha256`, complete flat `upsertCases`, and `removeCaseRefs`. Agent Case 保持 `executionMethods: [agent]` 与完整 `agentTestSpec`；不要把 endpoint、authentication 或 Agent Under Test 配置写入 Case。Do not submit a complete Candidate or any audit-internal model, data requirement, Finding, Confirmation, or Proposal.
...
6. Never map source Requirement IDs, rewrite Historical `requirementRefs`, or repair historical Traceability. Mapping advisories are Service-owned; if a current Requirement remains uncovered, add a genuinely current Case instead.
7. Repair Agent constraints by minimum change. Preserve the rule that partial order is not a complete Trace, and never turn missing Tool/MCP visibility into a fabricated expected absence; visibility gaps are handled by Assertion Engine as `NOT_EVALUABLE`.
