...
| 当前代码基线 | Phase 1～Phase 3 已实现；Phase 4 后端已实现，数据库迁移最高版本为 19，前端核心设计与发布闭环使用真实运行数据 |

本文给出第四期的实施级技术设计及当前落地边界。服务端领域协议、迁移 15～19、节点级固定工作流、四 Agent 受控运行、规范化持久化、人工门禁、审计、发布和执行交接已经实现；Phase 4 页面已接入真实任务、双门禁、各阶段产物、测试点树结构编辑与独立批准、用例/数据编辑审核、处置、重新审计、不可变发布与三种导出、项目目录/套件、冒烟候选、影响回归和执行交接。本期仍不表示已交付真实测试数据、脚本生成或测试执行能力。
...
测试设计任务包固定携带 `designContext` 与服务端版本化的 `generationPolicy`。前者包含测试目标、包含/排除范围、重点维度和用户覆盖目标；后者要求四个 Agent 先建立覆盖矩阵，再按独立角色、状态、输入等价类/边界、业务分支、失败原因和可观察结果拆分。该策略以独立可验证行为决定产物数量，不设置脱离输入规模的固定最低条数，也禁止同义重复凑数。

- TestAnalysis 按每个 coverage target 生成原子 coverage unit，显式保留正向、反向、边界、状态、权限、接口、数据副作用和 oracle；
- Functional 按“角色 × 状态 × 输入分区/边界 × 分支 × 入口 × 结果”发散最小测试点；
- NonFunctional 对性能、稳定性、兼容性、安全四分区逐一给出结论，并对适用分区按负载、故障、兼容矩阵或威胁场景拆分；
- Synthesis 先核对批准树中的全部适用节点，再按不同前置、数据、动作和结果拆分用例；仅当前置、动作、数据和最终判定高度一致时允许多测试点归并；
- `test_case_synthesis.submit_result` 在接收候选时校验批准适用测试点全集，存在遗漏即返回具体 nodeId 并要求完整重提，最终覆盖率仍只由服务端审计计算。
...
该方案保留 Phase 1～3 的固定输入、Evidence、Agent 配置、Tool 治理和 Worker 可靠性原则，并已落地 DAG、树编辑、用例 revision、数据需求、覆盖审计、项目测试资产聚合和机器可读交接能力。脚本编写、真实造数、`TestPlanVersion`、测试执行与基于结果发布新冒烟套件仍属于后续独立阶段；当前真实前端闭环不改变这一交付边界。
