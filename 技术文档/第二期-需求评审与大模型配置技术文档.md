...
资源目录通过 `ai_resources` 表或 JSON Store 持久化。MCP 目录当前只接受 HTTP/S Endpoint，不保存浏览器输入的凭据；Skill 依赖的工具必须已登记，MCP 来源工具必须引用已登记的 MCP 服务。Skill ZIP 最多 20 MB、200 个文件，单文件最多 5 MB、解压后总计最多 50 MB，且必须恰好包含一个非空 UTF-8 `SKILL.md`。服务端校验 ZIP CRC，拒绝绝对路径、盘符、空段、`.`、`..`、Windows 保留名、大小写冲突、符号链接和原生可执行文件，通过临时目录和同卷原子重命名写入 `SMARTHUB_SKILL_ROOT/{skillKey}/{version}`；资源保存压缩包 SHA-256、确定性内容 SHA-256、入口和文件清单。已上传包的 key、version、entrypoint、运行权限和 package 元数据不可通过通用更新接口覆盖；Agent 发布版本的 `configurationHash` 同时包含包内容 Hash 与运行权限，删除资源时同步清理包目录。
