...
> 本次变更：正文直传与 extraction/v3 已落地；需求点内嵌 Evidence 草稿，关联 ID 与引用由服务端生成；Evidence Normalizer 支持 Markdown 可见文本映射、同资产唯一 Chunk 重定位和可唯一定位的末尾省略号修复
...
1. JSON Schema 校验；
2. 枚举、长度、数量和唯一性校验；
3. `InputDeliveryManifest` 完整性、Evidence 草稿引用存在性与快照归属校验，并生成规范 locator/coverage；
...
- Evidence 仍必须回到固定 Chunk：服务端优先连续逐字匹配，必要时将 Markdown 可见文本确定性映射回连续原始字符范围，并生成规范 quote 与 locator；
- `knowledge.read_chunk` 只用于 Evidence 修复或局部补读，`knowledge.search` 只用于补充知识，不得恢复为正常规模正文的强制逐块读取链；
- 旧 `requirement-point-extraction/v1`、`v2` 运行保持只读兼容；新运行使用需求点内嵌 Evidence 草稿且不含模型自报 coverage 的 `requirement-point-extraction/v3`。
...
```json
{
  ...
  "agentDefinitions": {
    "requirementPointExtraction": {"versionId": "...", "sha256": "...", "resultSchemaVersion": "requirement-point-extraction/v3"},
    "requirementReview": {"versionId": "...", "sha256": "...", "resultSchemaVersion": "requirement-review/v2"}
  },
  ...
  "toolsets": {
    "requirementPointExtraction": [{"name": "evidence.validate_batch", "version": "1.0.0"}, {"name": "requirement-points.submit_result", "version": "3.0.0"}],
    "requirementReview": [{"name": "review.submit_result", "version": "3.0.0"}]
  },
  "mcpConfigVersionIds": [],
...
| 结果协议 | `requirement-point-extraction/v3` / `requirement-review/v2` |
...
2. `segmented_context`：超出预算时，按资产逻辑路径、标题结构和 Chunk 边界稳定分批。每批直接注入正文，要求生成带来源草稿；全部批次完成后，最后一次调用只接收批次草稿、全部 Evidence 草稿和跨文档关系索引，执行全局去重、归并与完整性复核；
...
- 需求点按可独立实现、测试或验收的粒度拆分，分别提供 `actor`、`action`、`object`、`conditions`、`businessRules`、`exceptions`、`acceptanceCriteria`；不同主体、动作、条件、状态、异常、权限、数据约束或验收项应拆分，只有语义重复项可归并，并保留全部来源 Evidence、`mergeGroupId` 与 `mergeRationale`；
- Evidence 草稿必须引用输入包中的固定资产和 Chunk，不得使用省略号或改写事实。服务端按“声明 Chunk 精确匹配 → Markdown 可见文本映射 → 同一固定资产唯一绝对位置重定位”的顺序规范化；只忽略 Markdown 结构标记和空白差异，不使用模糊语义匹配。跨资产、无匹配或多个不同原文位置均拒绝；少于 4 个可见字符的摘录不得提交；
- 工具白名单采用阶段化可见性：首次 `full_context` 提取和 `segmented_context` 最终归并只暴露 `evidence.validate_batch` 与 `requirement-points.submit_result`；服务端已返回 Evidence 校验问题且进入修复回合后，Runtime 才把 `knowledge.search/read_chunk` 加入当前 Pi tool context，并记录 `evidence_repair_tools_enabled` 事件。工具虽在 AgentDefinition 中注册，但未进入当前阶段 context 时仍由 `beforeToolCall` 拒绝；
...
- 只生成 Finding、风险、建议和评审摘要；每条 Finding 只能关联固定需求点，协议中不得提交直接 Evidence；原文依据由关联需求点的固定 Evidence 间接追溯；
- Finding 不因严重度要求直接 Evidence；没有问题时不得虚构 Finding；
- 必须通过 `review.submit_result` 结束，普通文本回答不构成正式结果。

### 10.4 工具集合

提取 Agent 使用下列补充知识/证据工具和自己的提交工具；正常 `full_context` 路径预期只调用批量 Evidence 预校验（可选）与提交工具。评审 Agent 只获得最后一行的评审提交工具：
...
| `evidence.validate_batch` | read | 一次校验多条 Evidence 草稿的快照归属，执行精确/Markdown 可见文本/同资产唯一位置解析，返回规范原文 quote、纠正后的 chunkId、locator、匹配策略或可修复问题；默认单批最多 100 条并受结果大小限制 | 是 | 不需要 |
| `requirement-points.submit_result` | internal_write | 提交 `requirement-point-extraction/v3`：每条需求点内嵌自己的 Evidence 草稿；服务端生成需求点/Evidence ID、引用、规范定位和 coverage | 每执行一次 | 不需要 |
| `review.submit_result` | internal_write | 只提交 Finding 与评审摘要并终止评审 Agent | 每执行一次 | 不需要 |

`knowledge.read_asset` 保留为兼容旧 v1 运行的内部工具，新 v3 AgentDefinition 不暴露它。重复调用默认拒绝；显式允许的只读固定快照工具可重放一次缓存成功结果。重放不改变 `InputDeliveryManifest`，不占用工具预算，并在 Pi 结果和审计中标记 `replayed=true`。
...
`requirement-point-extraction/v3` 候选协议把 Evidence 草稿直接嵌入所属需求点，不允许模型提交需求点 ID、Evidence ID、`evidenceRef(s)`、coverage 或最终 locator：
...
归并需求点额外使用 `mergeGroupId` 与 `mergeRationale`，二者必须同时存在并在需求点内部保留全部来源 Evidence 草稿。提交时服务端按数组顺序生成稳定 `RP-*`，按规范固定位置生成并去重 `E-*`，再从嵌套关系生成每条需求点的 `evidenceRefs`；相同固定原文可由多个需求点共享。服务端同时使用 `InputDeliveryManifest` 生成 `coverage`。任一引用、投递或归并校验失败时，提交工具返回带需求点下标和 Evidence 下标的可修复路径且不冻结结果。旧 v1/v2 只用于历史读取和迁移，不用于创建新运行。

`requirement-review/v2` 只包含：
...
第二段 Schema 设置 `additionalProperties: false`，不接受 `requirementPoints`、`evidence` 或 `coverage`。SmartHub 合并并再次校验后的正式结果结构如下：
...
3. 每批预留需求点草稿输出，最终归并调用预留全部草稿、Evidence 草稿和跨文档关系索引；
...
- 每条 Evidence 草稿必须引用投递清单中的固定资产，摘录至少四个可见字符，并能通过精确原文或 Markdown 可见文本映射回唯一连续原始字符范围；服务端生成规范 quote、chunkId、标题与字符区间，不信任模型 locator；
- 每个需求点必须含有效 Evidence、可独立实现/测试/验收的原子字段，并在语义重复归并时保留全部来源 Evidence、归并组和理由；
...
| FR-204 | Evidence Normalizer、固定 AssetVersion、大纲与定位 | Evidence 草稿回到固定 Chunk 并由服务端生成 locator，Finding→需求点→Evidence→原文联动 |
...
退出条件：可用固定快照运行 `RequirementAnalysisWorkflow`；正常规模输入首轮直接包含全部正文，超长输入按固定批次注入并最终归并，服务端根据投递 Manifest 与 Evidence 草稿冻结结果，再由独立评审 Agent 提交评审结果。
...
```json
{
  ...
  "resultSchemaVersion": "requirement-point-extraction/v3",
  "inputDeliveryPolicy": {
    "version": "1.0.0",
  ...
  "tools": [
    ...
    {"name": "evidence.validate_batch", "version": "1.0.0"},
    {"name": "requirement-points.submit_result", "version": "3.0.0"}
  ],
  "limits": {
    "maxTurns": 32,
    "maxToolCalls": 24,
