...
当前实现使用 `/agent-configurations/requirement-analysis` 维护需求点提取和需求评审两个 Agent 的独立草稿与发布版本。页面通过 `/ai-resources` 读取完整 Tool、MCP、Skill 目录。草稿分别保存 `toolIds`、`mcpServerKeys` 和 `skillKeys`；服务端返回 `requiredToolIds`、`requiredMcpServerKeys`、`requiredSkillKeys` 供页面固定必需项，其余已登记且启用的资源均可按 Agent 自由添加或移除。发布前强制校验 Skill 的依赖 Tool 已同时选择、MCP Tool 的所属 MCP 服务已同时选择且工具位于服务白名单。发布时自定义 Tool 的来源、路径/端点、风险、超时、凭据引用和参数 Schema 进入 Toolset Hash；MCP 规范化为 `{ serverKey, version, enabled, toolIds, policyHash }`，Skill 规范化为 `{ skillKey, version, enabled, configurationHash }`，并写入不可变 `AgentDefinitionVersion`。运行时按这些 Hash 读取 Skill 内容并装配 Local/HTTP/MCP Tool，目录后续改名或修改不会静默改变已发布 Agent 语义；发生漂移的扩展能力拒绝加载并产生可审计不可用事件。
