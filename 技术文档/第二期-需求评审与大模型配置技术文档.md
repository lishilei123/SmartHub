...
> 文档版本：V1.2
> 编制日期：2026-07-23  
> 对应需求：`需求文档/第二期-需求评审与大模型配置需求文档.md` V1.4
> 上位文档：`需求文档/需求总览.md` V1.5
...
2. **项目版本强隔离**：Review、Run、Finding、问答和统计必须带显式 `project_version_id`；不允许缺省到 latest/default，也不允许跨版本混查。
3. **输入固定**：一次 ReviewRun 必须固定需求绑定、资产、索引、Prompt、模型和工具版本。
4. **先校验后发布**：Agent 输出只能进入候选区，服务端校验通过后才能成为正式结果。
5. **工具最小权限**：Agent 只能看见当前版本显式授权的工具，执行仍需通过运行时策略。
6. **证据优先**：重要结论必须有可定位证据；模型生成的理由不能替代证据。
7. **人机边界清晰**：外部副作用操作默认需审批；Finding 的业务结论由人确认。
8. **可恢复执行**：模型、工具、Worker 异常不得造成重复副作用或晚到结果覆盖。
9. **渐进建设**：第二期不引入通用工作流平台，但接口和数据模型可支持后续多 Agent。
10. **只读评审工作台**：前端视图状态不得修改固定需求版本；更新需求必须通过一期知识库产生新资产版本。
11. **运行一致性优先**：列表、概览、原文、证据、功能树、Diff 和问答必须绑定同一显式 `projectVersionId + runId`，不得各自回退到 latest。
...
```text
/projects/{projectId}/versions/{projectVersionId}/requirement-reviews/{reviewId}/runs/{runId}
...
- 版本 Diff 只能比较同一 `assetId` 下两个可读固定版本，并显示双方 `projectVersionId/requirementBindingId/assetVersionId`；跨项目版本比较必须显式选择双方，与差异相关的 Finding 必须来自明确运行或服务端关联结果。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/projects/{projectId}/versions` | 查询项目版本；不隐式创建默认版本 |
| POST | `/projects/{projectId}/versions` | 创建 open 版本，可带 `baseProjectVersionId` 仅继承有效需求绑定 |
| GET | `/projects/{projectId}/versions/{projectVersionId}` | 查询版本状态和摘要 |
| POST | `/projects/{projectId}/versions/{projectVersionId}/lock` | 锁定版本，之后只读 |
| POST | `/projects/{projectId}/versions/{projectVersionId}/archive` | 归档版本 |
| GET | `/projects/{projectId}/versions/{projectVersionId}/requirements` | 查询当前版本有效需求绑定及其 Review 摘要 |
| POST | `/projects/{projectId}/versions/{projectVersionId}/requirements` | 绑定知识库中一个 `ready` 的固定需求资产版本 |
| PUT | `/projects/{projectId}/versions/{projectVersionId}/requirements/{bindingId}` | 以新 `ready` 资产版本替换；旧绑定 superseded，新建绑定 |
| POST | `/projects/{projectId}/versions/{projectVersionId}/requirement-uploads` | 上传 `.md/.txt/.zip`，复用一期入库 Job，并在 ready 后绑定发起版本 |
...
| GET | `/reviews?projectId={projectId}&projectVersionId={projectVersionId}` | 按强制项目版本作用域、关键字、运行状态、未处理 Finding 查询 |
...
服务端通过 `requirementBindingId` 解析并固定 `assetId/assetVersionId`，客户端不得用任意资产版本覆盖绑定。绑定不属于 `projectVersionId`、已 superseded 或版本非 open 时拒绝创建。同一用户、同一路径和相同 `Idempotency-Key` 在有效期内必须返回相同创建结果；请求体摘要不同则返回 `409 IDEMPOTENCY_KEY_REUSED`。
