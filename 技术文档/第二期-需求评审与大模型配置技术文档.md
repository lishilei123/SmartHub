...
- 当前 Agent 后台执行驻留在 API 服务进程内；服务成功重启后会将上一进程遗留的 `running` ReviewRun 标记为 `REVIEW_RUN_INTERRUPTED`，持久化 Worker 租约与断点恢复仍属于后续差距；
...
| 正式 Review Job/Worker、Finding 人工处置与问答闭环仍需按里程碑完善 | 继续复用租约/心跳/fencing 算法，不以当前直接运行或前端本地状态替代正式业务闭环 |
| 检索默认可能使用活动索引 | 增加按固定 `index_version_id` 检索，评审执行期间禁止漂移到 latest |
| 模型来源注册已存在但正式路由/版本治理仍不完整 | 完善 Model Registry、Model Router、Model Gateway 和不可变配置版本 |
| 内置 Agent 定义已存在但持久化发布治理仍不完整 | 完善不可变 `agent_definition_versions`、正文投递策略版本和发布流程 |
| Tool Runtime 基础门禁已存在但审批/外部工具治理仍不完整 | 完善 Tool Policy Engine、Approval Gate、MCP/联网治理 |
| 结构化候选与校验已存在但正式 Finding 业务闭环未完成 | 完成正式结果原子发布、Finding 处理记录和问答 |
