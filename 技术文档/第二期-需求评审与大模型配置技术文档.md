...
> 文档版本：V1.0  
> 编制日期：2026-07-23  
> 对应需求：`需求文档/第二期-需求评审与大模型配置需求文档.md` V1.2  
...
`review_chat_sessions` 必须绑定一个已完成 ReviewRun；`review_chat_messages` 保存用户问题、回答、引用和模型调用引用。追问只能读取该运行快照与结果，不能自动切换到最新需求或最新知识索引。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/review-runs/{runId}/findings` | 筛选问题 |
...
| 需求 | 技术落点 | 主要验证 |
| --- | --- | --- |
| FR-201 | Model Registry、来源 API、Secret Ref | 来源增删改查、秘密不回显 |
| FR-202 | Provider Factory、Pi AI、能力探测 | 内置/兼容来源合约测试 |
| FR-203 | Model Registry、capabilities、health | 模型启停和能力过滤 |
| FR-204 | ModelConfigVersion、Route Entries | 草稿校验、发布不可变 |
| FR-205 | Model Router | 优先级、限流、故障降级 |
| FR-206 | ModelCall/Usage | token、延迟、成本可查询 |
| FR-207 | Credential Ref、日志脱敏 | API/日志/Prompt 泄漏测试 |
| FR-208 | Model Config UI/API | 配置权限和激活流程 |
| FR-301 | Review/Run/Application Service | 创建、列表、详情和幂等 |
| FR-302 | RunSnapshot、固定资产和索引 | 运行期间配置不漂移 |
| FR-303 | Result/Finding/Evidence | 结构化结果和证据定位 |
| FR-304 | ReviewWorker、状态机和取消 | 排队、执行、失败、取消 |
| FR-305 | FindingAction | 接受/驳回/转任务/解决历史 |
| FR-306 | Follow-up Session | 绑定旧快照的追问和引用 |
...
| 验收 | 验证场景 |
| --- | --- |
| AC-201 | 至少配置两类来源，探测、启停、秘密不回显，路由正确 |
| AC-202 | 发布默认/备用模型配置；主模型 429 后按策略切换并显示原因 |
| AC-203 | 创建 Review 后固定资产、索引、Agent、模型和工具版本 |
| AC-204 | RequirementAnalysisAgent 使用受控工具完成一次真实评审 |
| AC-205 | 输出包含结构化 Finding、严重级别、置信度、建议和有效证据 |
| AC-206 | 用户可取消、重试、查看阶段和稳定错误码；崩溃后可恢复 |
| AC-207 | Finding 人工处理有权限、并发保护和完整追加历史 |
...
```text
DATABASE_URL
SMARTHUB_PROCESS_ROLE=api|knowledge-worker|review-worker
SMARTHUB_SECRET_PROVIDER=env|vault|...
SMARTHUB_REVIEW_WORKER_CONCURRENCY=2
SMARTHUB_AGENT_MAX_TURNS=12
SMARTHUB_AGENT_MAX_TOOL_CALLS=40
SMARTHUB_AGENT_DEADLINE_MS=900000
SMARTHUB_MODEL_CALL_TIMEOUT_MS=120000
SMARTHUB_TOOL_DEFAULT_TIMEOUT_MS=30000
SMARTHUB_REVIEW_EVENT_RETENTION_DAYS=90
SMARTHUB_EXTERNAL_EVIDENCE_RETENTION_DAYS=30
```

模型来源密钥使用独立命名或 Secret Manager path，由 `credential_ref` 间接引用，不把密钥写进通用 JSON 配置文件。
