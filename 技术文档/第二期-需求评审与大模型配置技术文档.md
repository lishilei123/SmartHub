...
第二期的需求分析使用固定的两阶段串联，不提供任意 DAG 或动态多 Agent 编排。每个阶段都有独立 `agent_type`、`agent_definition_version_id`、`execution_id` 和 Pi session id。
...
当前实现使用 `/agent-configurations/requirement-analysis` 维护需求点提取、需求评审和评审问答三个 Agent 的独立草稿与发布版本。`reviewQa` 映射到运行时 `review-qa`，内置结果协议为 `review-qa/v1`，必需工具为 `review.answer_submit@1.0.0`（Pi 名称 `review_answer_submit`）。页面通过 `/ai-resources` 读取完整 Tool、MCP、Skill 目录。草稿分别保存 `toolIds`、`mcpServerKeys` 和 `skillKeys`；服务端返回 `requiredToolIds`、`requiredMcpServerKeys`、`requiredSkillKeys` 供页面固定必需项，其余已登记且启用的资源均可按 Agent 自由添加或移除。发布前强制校验 Skill 的依赖 Tool 已同时选择、MCP Tool 的所属 MCP 服务已同时选择且工具位于服务白名单。发布时自定义 Tool 的来源、路径/端点、风险、超时、凭据引用和参数 Schema 进入 Toolset Hash；MCP 规范化为 `{ serverKey, version, enabled, toolIds, policyHash }`，Skill 规范化为 `{ skillKey, version, enabled, configurationHash }`，并写入不可变 `AgentDefinitionVersion`。运行时按这些 Hash 读取 Skill 内容并装配 Skill 脚本/网络、Local、HTTP 和 MCP Tool，目录后续修改不会静默改变已发布 Agent 语义；发生漂移的扩展能力拒绝加载。`skill_execute_script` 的模型参数以 `script + args` 为主，`skillKey` 为可选消歧字段；服务端只在当前发布快照已绑定的 Skill 中按运行清单精确匹配脚本路径，零匹配拒绝、唯一匹配自动补齐、多个匹配要求显式 `skillKey`，因此模型无需维护服务端绑定引用且不会扩大脚本权限。评审问答始终以成功 ReviewRun 的固定资产、正式结果和 Evidence 白名单作为事实边界，外部能力返回内容不得成为 `citations`；服务端只接受该运行内的 Evidence ID。
