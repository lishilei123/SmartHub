...
> 文档版本：V1.5
> 编制日期：2026-07-26
> 对应需求：`需求文档/第二期-需求评审与大模型配置需求文档.md` V1.8
> 上位文档：`需求文档/需求总览.md` V1.8
> 技术基线：第一期知识库、PostgreSQL/pgvector、独立 Worker、Pi Agent Framework
> 本次变更：引入 Context Assembler、完整/分批正文直传、InputDeliveryManifest、批量 Evidence 校验和 extraction/v2；当前代码仍待按本文迁移
...
- Evidence 仍必须回到固定 Chunk 并由服务端连续逐字校验、生成 locator；
...
- Evidence 草稿必须引用输入包中明确标识的固定 Chunk 并连续逐字复制，禁止拼接非连续原文或使用省略号；服务端校验失败后只对失败项定点补读或批量预校验，未通过校验、少于 4 个字符的摘录不得提交；
...
| `evidence.validate_batch` | read | 一次校验多条 Evidence 草稿的快照归属与连续逐字引用，返回每项规范 locator 或可修复问题；默认单批最多 100 条并受结果大小限制 | 是 | 不需要 |
...
- 每条 Evidence 草稿必须引用投递清单中的固定 Chunk，摘录至少四个字符、连续逐字可重新定位；服务端生成标题与字符区间，不信任模型 locator；
