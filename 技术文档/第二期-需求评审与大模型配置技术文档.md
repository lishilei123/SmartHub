...
> 文档版本：V1.1
> 编制日期：2026-07-23  
> 对应需求：`需求文档/第二期-需求评审与大模型配置需求文档.md` V1.3
> 上位文档：`需求文档/需求总览.md` V1.4  
...
3. 如何保证每次评审使用固定的需求资产、索引、Prompt、模型与工具版本，可复现、可追溯；
...
2. **输入固定**：一次 ReviewRun 必须固定资产、索引、Prompt、模型和工具版本。
3. **先校验后发布**：Agent 输出只能进入候选区，服务端校验通过后才能成为正式结果。
4. **工具最小权限**：Agent 只能看见当前版本显式授权的工具，执行仍需通过运行时策略。
5. **证据优先**：重要结论必须有可定位证据；模型生成的理由不能替代证据。
6. **人机边界清晰**：外部副作用操作默认需审批；Finding 的业务结论由人确认。
7. **可恢复执行**：模型、工具、Worker 异常不得造成重复副作用或晚到结果覆盖。
8. **渐进建设**：第二期不引入通用工作流平台，但接口和数据模型可支持后续多 Agent。
9. **只读评审工作台**：前端视图状态不得修改固定需求版本；更新需求必须通过一期知识库产生新资产版本。
10. **运行一致性优先**：列表、概览、原文、证据、功能树、Diff 和问答必须绑定同一显式 `runId`，不得各自回退到 latest。
...
- `requirement_asset_version_id` 和内容摘要；
...
保存用户的评审业务对象：项目、项目版本、标题、需求资产选择、评审范围、创建人、当前最新运行和汇总状态。Review 不保存可变的 Agent 对话状态。

#### 8.2.6 `review_runs` 与 `review_run_snapshots`
...
- 优先按“模型配置 → Agent 配置 → Review/Job → 执行审计 → 结果/证据”拆分迁移；
...
1. `succeeded` 的 ReviewRun 必须且只能有一个 Published Result；
2. 正式 Finding 必须属于该 Run 的正式 Result；
3. 内部 Evidence 的资产和索引必须与 RunSnapshot 一致；
4. 一个 Run 同时最多一个有效 lease 和一个 active AgentExecution；
5. 不携带当前 fencing token 的结果写入一律拒绝；
6. 取消确认后，不接受晚到的模型或工具结果；
7. 发布后的 Agent/Model 配置版本不可修改；
8. 追问不得改变原评审结果，只能创建新的消息与引用记录；
9. 工具审批记录不得跨工具、跨参数、跨 ReviewRun 使用；
10. 用户看到的成功状态只在结果事务提交后出现。
...
可启用供应商 Prompt Cache 或前缀缓存以降低固定 Prompt 成本，但它只优化模型输入，不缓存评审最终结果。缓存键至少包含 Agent 定义摘要、工具 Schema 摘要、模型和协议版本；不得把不同项目的敏感上下文错误复用。
...
sequenceDiagram
    ...
    U->>API: 创建 Review + 选择需求资产/范围
    API->>APP: createReview()
    APP->>DB: 校验项目权限、资产和可用索引
...
页面采用“顶部操作区 + 左侧评审列表 + 中间评审工作区 + 右侧评审问答区”。全局产品导航不计入本页面三栏。

| 区域 | 主要组件 | 数据来源 |
| --- | --- | --- |
| 顶部操作区 | `ReviewHeader`、`RunStatus`、`RunActions`、`SnapshotSummary` | Review、所选 ReviewRun、运行事件与快照摘要 |
| 左侧列表 | `ReviewList`、筛选器、`RunHistoryPicker` | `/reviews`、`/reviews/{id}` |
...
```text
/projects/{projectId}/requirement-reviews/{reviewId}/runs/{runId}
  ?view=overview|source|diff|tree|evidence
  &findingId={findingId}
  &evidenceId={evidenceId}
```

- `reviewId`、`runId` 和 `view` 是可恢复状态；刷新、前进后退和复制链接必须保持同一历史运行。
- `findingId/evidenceId` 用于深链接和联动定位；目标不可访问时保留当前运行并显示错误，不回退到最新版本。
- 搜索、筛选、折叠、滚动位置和临时高亮是视图状态；不得写入 ReviewRun、Finding 或 Evidence。
- 切换 `runId` 时取消旧请求，清空仅属于旧运行的选中项和消息草稿，再并行加载新运行数据；所有响应在提交到 UI 前校验响应 `runId`，防止竞态串屏。

#### 14.7.3 首屏加载与真实进度

1. 读取 Review 汇总与运行历史，确认所选 `runId` 属于当前 Review；
2. 读取 Run 状态、阶段、进度和快照摘要；
3. 成功运行并行读取正式 Result、Findings 和当前视图所需数据；
4. 原文和证据按固定 `assetVersionId` 懒加载，不预取资产最新版本；
5. 运行中通过增量事件或轮询更新阶段，断线后重新读取服务端 Run 状态和事件游标；
6. 只有服务端正式 Result 参与统计和列表，候选结果、流式文本和本地计数不进入完成态页面。
...
- 资产版本历史、ReviewRun 历史和 Finding Action 历史分别读取和展示，UI 不合并成含义不明的单一版本列表。
- 版本 Diff 只能比较同一 `assetId` 下两个可读固定版本，并显示比较双方 ID；与差异相关的 Finding 必须来自明确运行或服务端关联结果。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/reviews` | 创建 Review 和首个 Run |
| GET | `/reviews` | 按项目、版本、关键字、运行状态、是否存在未处理 Finding 查询 |
| GET | `/reviews/{id}` | Review 汇总与运行历史 |
| POST | `/reviews/{id}/runs` | 使用指定配置创建新 Run |
...

{
  ...
  "title": "第二期需求评审",
  "requirementAssetVersionIds": ["assetv_101"],
  "scope": {
...
同一用户、同一路径和相同 `Idempotency-Key` 在有效期内必须返回相同创建结果；请求体摘要不同则返回 `409 IDEMPOTENCY_KEY_REUSED`。

### 15.5 Findings、Evidence 与追问
...
前端读取原文复用一期 `GET /api/asset-versions/{assetVersionId}`，不得新增只接收 `assetId` 并自动解析 latest 的评审专用接口。提交追问时可携带 `findingIds` 和结构化 `sourceQuotes`；服务端校验它们属于会话绑定的 RunSnapshot 或其允许证据范围，并验证摘录 Hash。
...
`request_id → review_id → review_run_id → job_id → agent_execution_id → model_call_id/tool_call_id`
...
- 切换历史 `runId` 后，workspace、Finding、证据、固定原文和问答不串用其他运行数据；
...
- 页面不存在 AI 改写、自动应用、测试用例生成、多格式解析和 Jira/PingCode 推送入口。
...
| 需求 | 技术落点 | 主要验证 |
| --- | --- | --- |
| FR-201 | Review/Run Application Service | 仅从 ready 固定需求版本创建评审，禁止绕过知识库 |
| FR-202 | RunSnapshot、ReviewWorker、事件流 | 固定运行快照，真实阶段、取消和历史运行不覆盖 |
...
| FR-207 | Review/Run/AssetVersion 历史 | 过期标记、历史切换和三类历史分离 |
| FR-208 | 上下文预算、检索降级 | 超长拒绝、降级显示和无索引失败 |
| FR-209 | Review Workspace、路由状态、报告模板 | 三栏工作台、断线恢复、只读边界和 Markdown 导出 |
...
| 验收 | 验证场景 |
| --- | --- |
| AC-201 | 仅从 ready 固定需求版本创建评审，运行快照不随新版本变化 |
...
| AC-209 | 三栏工作台绑定同一 runId，Finding/原文双向定位、引用追问、真实进度和安全导出均通过 E2E |
