...
> 本次变更：正文直传与 extraction/v2 已落地；Evidence Normalizer 新增 Markdown 可见文本到原文的确定性映射及同资产唯一 Chunk 重定位
...
- 旧 `requirement-point-extraction/v1` 运行保持只读兼容；新运行使用不含模型自报 coverage 的 `requirement-point-extraction/v2`。
...
```json
{
  ...
  "agentDefinitions": {
    "requirementPointExtraction": {"versionId": "...", "sha256": "...", "resultSchemaVersion": "requirement-point-extraction/v2"},
  ...
  "toolsets": {
    "requirementPointExtraction": [{"name": "evidence.validate_batch", "version": "1.0.0"}, {"name": "requirement-points.submit_result", "version": "2.0.0"}],
...
| 结果协议 | `requirement-point-extraction/v2` / `requirement-review/v2` |
...
| `requirement-points.submit_result` | internal_write | 提交 `requirement-point-extraction/v2` 的需求点和 Evidence 草稿；服务端规范化 Evidence 并生成 coverage | 每执行一次 | 不需要 |
| `review.submit_result` | internal_write | 只提交 Finding 与评审摘要并终止评审 Agent | 每执行一次 | 不需要 |

`knowledge.read_asset` 保留为兼容旧 v1 运行的内部工具，新 v2 AgentDefinition 不暴露它。重复调用默认拒绝；显式允许的只读固定快照工具可重放一次缓存成功结果。重放不改变 `InputDeliveryManifest`，不占用工具预算，并在 Pi 结果和审计中标记 `replayed=true`。
...
`requirement-point-extraction/v2` 候选协议不允许模型提交 coverage 或最终 locator，只包含：
...
归并需求点额外使用 `mergeGroupId` 与 `mergeRationale`，二者必须同时存在并保留全部来源 Evidence。提交时服务端把 Evidence 草稿规范化为正式 `evidence`，使用 `InputDeliveryManifest` 生成 `coverage`；任一引用、投递或归并校验失败时，提交工具返回按 Evidence/需求点分组的可修复问题且不冻结结果。旧 `requirement-point-extraction/v1` 只用于历史读取和迁移，不用于创建新运行。
...
```json
{
  ...
  "resultSchemaVersion": "requirement-point-extraction/v2",
  "inputDeliveryPolicy": {
    "version": "1.0.0",
  ...
  "tools": [
    ...
    {"name": "requirement-points.submit_result", "version": "2.0.0"}
  ],
  "limits": {
    "maxTurns": 32,
    "maxToolCalls": 24,
