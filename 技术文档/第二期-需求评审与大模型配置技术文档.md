...
四个内置工具分别维护独立实现文件，由 `requirement-tools.ts` 组合为需求点提取和需求评审两个运行时注册表，目录中的每个内置工具固定映射自身源码文件；自定义本地工具显式登记相对源码路径。源码读取 API 仅允许 `server/tools` 和 `ai/tools` 目录内不超过 512 KB 的 TypeScript/JavaScript 文件，使用 `realpath` 复验实际文件仍位于允许目录，从而拒绝绝对路径、`..`、符号链接越界和任意文件读取。目录登记属于治理元数据，不自动注册运行时 Handler，也不自动写入 Agent 白名单；运行时仍以服务端 Tool Registry、Agent 发布版本和固定运行快照为准。
...
当前实现使用 `/agent-configurations/requirement-analysis` 维护需求点提取和需求评审两个 Agent 的独立草稿与发布版本。页面通过 `/ai-resources` 读取完整 Tool、MCP、Skill 目录，不再使用按 Agent 硬编码的局部展示列表。草稿分别保存 `toolIds`、`mcpServerKeys` 和 `skillKeys`；服务端返回 `requiredToolIds`、`requiredMcpServerKeys`、`requiredSkillKeys` 供页面固定必需项，其余已登记且启用的资源均可按 Agent 自由添加或移除。发布时 Tool 标识及资源版本参与 Toolset Hash，MCP 规范化为 `{ serverKey, version, enabled, toolIds, policyHash }`，Skill 规范化为 `{ skillKey, version, enabled, configurationHash }`，并写入不可变 `AgentDefinitionVersion`。历史版本不随资源目录后续改名、升级或停用而变化。目录选择不自动注册 Handler 或扩大权限；Pi Runtime 只向模型暴露当前阶段真实注册的 Tool，其他选择保留为可审计绑定并产生不可用事件，不再导致整个 Agent 运行失败。
