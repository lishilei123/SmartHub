...
- “需求点”视图取代“功能树”。`RequirementPoint` 至少保存稳定 ID、标题、说明和固定 Evidence 引用；Finding 通过 `requirementPointRefs` 关联一个或多个需求点。
...
- 点击需求点时只展示正式 `requirement_point_finding` 与 `requirement_point_evidence` 关系；点击 Evidence 后根据其 `assetVersionId` 选择左侧对应文件并定位原文，无关系时不创建前端伪关联。
