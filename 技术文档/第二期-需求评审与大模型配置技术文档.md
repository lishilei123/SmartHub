...
唯一约束：`(project_id, version_key)`。只有 `open` 状态允许新增/替换绑定和创建 ReviewRun；状态只允许 `open → locked → archived`，不在第二期提供反向解锁。删除是独立的受控操作：先删除该版本需求绑定，再删除版本；存在派生项目版本或需保留的 ReviewRun 审计记录时拒绝物理删除。
...
- 每条 Finding 至少通过 `requirementPointRefs` 关联一个有效需求点，其 `evidenceRefs` 必须来自关联需求点；工具错误、证据验证失败或工具成功本身不是需求 Finding；
...
Pi Runtime 必须以 assistant `stopReason=error` 和脱敏供应商错误识别模型请求失败。结果提交阶段的 429、临时 5xx 或连接错误采用最多两次指数退避与 jitter；重试耗尽分别终止为 `MODEL_RATE_LIMITED` 或 `MODEL_PROVIDER_UNAVAILABLE`。只有模型请求成功结束、候选结果不存在且没有调用 `review_submit_result` 时，才允许使用 `MODEL_TOOL_CALL_REQUIRED`；供应商错误不得降级模型的工具调用健康状态。
...
```json
{
  ...
  "tools": [
    ...
    {"name": "knowledge.read_asset", "version": "2.1.0"},
    {"name": "knowledge.read_chunk", "version": "1.0.0"},
    {"name": "evidence.validate", "version": "1.1.0"},
    {"name": "review.submit_result", "version": "2.0.0"}
  ],
  "limits": {
    "maxTurns": 24,
    "maxToolCalls": 40,
