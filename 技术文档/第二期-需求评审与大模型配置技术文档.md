...
4. 如何先从多文档中提取和归并需求点，再将 Agent 输出转化为需求点关联的结构化问题、证据引用和人工处理记录；
...
| 结果协议 | `requirement-point-extraction/v1` / `requirement-review/v1` |
...
- 必须先从全部固定输入文档提取、去重和归并需求点，不得只读取默认首段就宣称完成全文评审；每份输入文档至少有一条被需求点引用的固定证据；
- 引用必须从固定 Chunk 连续逐字复制，禁止拼接非连续原文或使用省略号；校验失败后先读取 Chunk，不得原样重试；
- 不得生成 Finding、评分、风险、建议或评审结论；
- 不得伪造文件、段落、URL、工具结果或执行状态；
- 必须通过 `requirement-points.submit_result` 结束，普通文本回答不构成正式结果。

评审 Agent Prompt 必须明确：

- 输入的 `requirementPoints`、`evidence` 和 `coverage` 已校验冻结，只读且不可重写；
- 只生成 Finding、风险、建议和评审摘要；每条 Finding 只能关联固定需求点，证据只能来自该需求点已有引用；
- 每个高/严重等级 Finding 至少一个有效证据；没有问题时不得虚构 Finding；
...
| `knowledge.read_asset` | read | 按 `assetVersionId` 分页读取固定资产目录/片段；指定行范围时默认不重复返回目录 | 是 | 不需要 |
| `knowledge.read_chunk` | read | 按 chunk id 读取上下文 | 是 | 不需要 |
| `evidence.validate` | read | 检查连续逐字引用和快照归属；失败返回原因/修复动作，成功返回精确 locator | 是 | 不需要 |
...
```json
{
  "requirementPoints": [],
  "evidence": [],
  "coverage": {"reviewedAreas": [], "notReviewedAreas": [], "limitations": []}
}
```

`requirement-review/v1` 只包含：
...
| GET | `/review-runs/{runId}/requirement-points` | 读取跨文档提取归并的需求点及其 Finding/Evidence 关系 |
