...
> 文档版本：V1.4
> 编制日期：2026-07-24
> 对应需求：`需求文档/第二期-需求评审与大模型配置需求文档.md` V1.6
> 上位文档：`需求文档/需求总览.md` V1.7
...
3. 如何以项目版本作为强制隔离边界，将多份需求绑定作为同一运行的固定输入集合，并保证资产、索引、Prompt、模型与工具版本可复现、可追溯；
4. 如何先从多文档中提取和归并需求点，再将 Agent 输出转化为需求点关联的结构化问题、需求点 Evidence 与人工处理记录；
...
1. JSON Schema 校验；
2. 枚举、长度、数量和唯一性校验；
3. 需求点 Evidence 引用存在性与快照归属校验；
4. Finding 与固定需求点关联校验；
5. 安全内容和敏感信息检查；
6. 结果质量门禁；
7. 单事务写入 ReviewResult、需求点、Finding 和需求点 Evidence 关系。
...
| 结果协议 | `requirement-point-extraction/v1` / `requirement-review/v2` |
...
完整文档不默认一次性塞入 Prompt。提取 Agent 接收服务端固定的资产—Chunk 覆盖计划与最小工具预算，必须先逐资产完整遍历分页目录，再按计划逐个读取所有未排除正文 Chunk；评审 Agent 只接收已冻结的提取结果。目录、Chunk 和 Evidence 的实际工具读取由 Runtime 审计，不能以模型文字、摘要或自行填写的 coverage 替代。长文采用这一固定分段读取和跨段归并方式，不因全部原文总 Token 预先拒绝；仅当目录、Chunk、Evidence 校验和提交的最小工具预算超过部署上限时，带资产数、Chunk 数和调用数拒绝启动。
...
- 必须完整遍历每份固定输入资产的分页目录，随后读取服务端 Coverage Plan 中每个未排除 Chunk；`coverage.assets` 只能记录实际读取的 `reviewedChunkIds`，预排除 Chunk 必须带快照中逐字一致的 `skippedChunks.reason`，缺页、漏读或伪造覆盖均不能冻结；
...
- 引用必须从实际读取的固定 Chunk 连续逐字复制，禁止拼接非连续原文或使用省略号；Evidence 校验失败后必须修正 quote、Chunk 或资产版本并重新校验，未通过校验、少于 4 个字符的摘录不得提交；
...
提取 Agent 使用下列只读知识/证据工具和自己的提交工具；评审 Agent 只获得最后一行的评审提交工具：

| 工具 | 风险 | 作用 | 幂等性 | 默认审批 |
| --- | --- | --- | --- | --- |
| `knowledge.search` | read | 在固定索引版本内检索 | 是 | 不需要 |
| `knowledge.read_asset` | read | 按 `assetVersionId` 分页读取固定资产目录/片段；`outlinePage.hasMore=true` 时必须以连续 offset 读取后续页。Runtime 记录每页 offset、数量、总数和 hasMore | 是 | 不需要 |
| `knowledge.read_chunk` | read | 按 chunk id 读取固定正文；Runtime 仅在首次成功调用后将 Chunk 记为已读。达到相同参数重复阈值时仅可重放一次本次执行的成功快照结果，不再次派发底层读取或新增读取证明；其余重复调用返回不可重试的策略错误 | 是 | 不需要 |
| `evidence.validate` | read | 检查连续逐字引用和快照归属；引用 Chunk 必须已被本运行读取，失败返回原因/修复动作，成功返回精确 locator | 是 | 不需要 |
| `requirement-points.submit_result` | internal_write | 提交需求点、固定证据和覆盖范围并终止提取 Agent | 每执行一次 | 不需要 |
| `review.submit_result` | internal_write | 只提交 Finding 与评审摘要并终止评审 Agent | 每执行一次 | 不需要 |

重复调用默认拒绝，不根据一般 `read` 或幂等声明自动重放。只有显式声明 `replay_success_once`、同时属于 `read` 且幂等的固定快照工具可重放一次缓存成功结果；写入、联网、未分类工具、失败调用及后续同参调用始终拒绝。重放不占用工具预算，并在 Pi 结果和审计中标记 `replayed=true`。
...
`requirement-point-extraction/v1` 只包含：
...
归并需求点额外使用 `mergeGroupId` 与 `mergeRationale`，二者必须同时存在并保留全部来源 Evidence。服务端用固定 Coverage Plan 校验每份资产目录分页的连续性、所有未排除 Chunk 的实际读取、预排除理由和 Evidence 来源 Chunk 的实际读取；任一校验失败时提交工具返回可修复问题且不冻结结果。
...
| 最大 Agent turns | 24 | 最后 3 turns 强制提交或修正当前结论；仍无有效结果则失败 |
...
工具调用总额度必须为结果提交保留独立配额。每个 Agent 的 `maxToolCalls` 中最后 3 次只允许该 Agent 的提交工具使用；提取阶段的普通读取或证据工具达到普通额度后，Pi Runtime 立即记录 `result_submission_required` 并强制调用 `requirement-points.submit_result`。评审阶段同理强制调用 `review.submit_result`。
...
提取结果冻结前，服务端依次执行 Schema、运行范围、目录覆盖、Chunk 覆盖、Evidence 与原子性校验：

- 每份固定资产的 `knowledge.read_asset` 目录页必须从 offset `0` 连续遍历到 `outlinePage.hasMore=false`；Runtime 审计页 offset、返回数量、总数和 hasMore，模型填充 coverage 不构成目录覆盖；
- Coverage Plan 中每个未排除 Chunk 必须有一次成功的 `knowledge.read_chunk` 记录并出现在所属资产 `reviewedChunkIds`；仅服务端生成的 `excludedReason` 可写入 `skippedChunks`，且理由必须逐字一致；
- 每条 Evidence 必须来自本运行实际读取的固定 Chunk，摘录至少四个字符、连续逐字可重新定位，且标题与字符区间必须等于工具校验的 locator；
...
sequenceDiagram
    ...
    O->>E: 新 session 启动 RequirementPointExtractionAgent
    loop 提取模型与只读工具回合
      E-->>O: events/checkpoints
      O->>DB: 状态、调用和心跳
    end
    E->>V: submit requirement points/evidence/coverage
    V->>DB: 校验证据、覆盖并冻结提取结果
...
黄金集按版本保存脱敏原文、Coverage Plan、人工标注的原子需求点、关键需求点、有效 Evidence 与可接受归并组。CI 使用固定模型/工具桩计算目录遍历率、未排除 Chunk 覆盖率、Evidence 可定位率、总体需求点召回率、关键需求点召回率、归并准确率和关键字段 Evidence 支撑率；试点阈值依次为 100%、100%、100%、90%、95%、90%、95%。模型、Prompt、检索或工具版本变更使关键需求点召回率下降超过 2 个百分点时阻断发布，除非记录具名人工豁免。
...
| FR-203 | Extraction Coverage Plan、Result Validator、RequirementPoint | 原子需求点、目录/Chunk 读取证明、固定 Evidence、归并信息和正式结果统计 |
| FR-204 | Evidence、固定 AssetVersion、大纲与定位 | Evidence 来源 Chunk 实际读取、Finding→需求点→Evidence→原文联动，不回退 latest |
...
| FR-208 | Chunk 分段读取、工具预算、检索降级 | 超长资料按计划读取；预算不足显示资产/Chunk/调用数并拒绝，无索引失败 |
...
| FR-401 | PiAgentRuntime、RequirementAnalysisWorkflow | 一 Run 依次执行提取与评审两个独立 Agent，目录/Chunk/Evidence 读取与冻结交接均可审计 |
...
| AC-202 | 原子需求点、归并来源与理由、目录/Chunk 覆盖状态均可审计；Finding 经关联需求点定位固定原文，关联需求点无 Evidence 时明确提示 |
| AC-203 | Finding 处置追加保存且不覆盖初始结果，新运行不覆盖旧运行 |
| AC-204 | 取消、模型失败和检索降级均显示真实状态，迟到结果不能发布 |
| AC-205 | 模型配置发布不可变，默认/备用路由、降级原因和秘密不回显均正确 |
| AC-206 | 模型检测反馈真实；目录缺页、Chunk/Evidence 未实际读取、伪造 coverage、无理由跳过、原子字段或归并信息缺失均不能生成成功统计或正式 Finding |
| AC-207 | RequirementAnalysisWorkflow 的两个 Agent 均受固定版本和工具权限约束；提取覆盖、原子性与证据校验完成冻结后，评审才可发布结果 |
...
退出条件：可用固定快照运行 `RequirementAnalysisWorkflow`，证明提取 Agent 完整遍历目录并读取计划 Chunk 后冻结候选结果，再由独立评审 Agent 提交评审结果。
...
| 长需求导致上下文超限 | 固定目录/Chunk 分段读取与最小工具预算，不按全文总 Token 截断 | 黄金样本验证覆盖与召回；预算不足显示拆分建议 |
| 覆盖宣称与实际读取不一致 | Runtime 记录目录页、Chunk 和 Evidence 来源，服务端按 Coverage Plan 校验 | 覆盖缺口、伪造 coverage、目录缺页和未读 Evidence 的确定性测试 |
...
```json
{
  ...
  "resultSchemaVersion": "requirement-point-extraction/v1",
  "prompt": {
  ...
  "tools": [
    ...
    {"name": "knowledge.read_asset", "version": "2.2.0"},
    {"name": "knowledge.read_chunk", "version": "1.0.0"},
    {"name": "evidence.validate", "version": "1.1.0"},
    {"name": "requirement-points.submit_result", "version": "1.0.0"}
  ],
  "limits": {
    "maxTurns": 32,
    "maxToolCalls": 100,
