...
- 不能把“未找到证据”描述为“已确认事实”；
- 必须先从全部固定输入文档提取、去重和归并需求点；每个需求点至少引用一条属于本次输入集合的固定证据；
- 每条 Finding 至少通过 `requirementPointRefs` 关联一个有效需求点；
...
| `knowledge.read_asset` | read | 按 `assetVersionId` 读取本次固定输入集合中某份资产的目录/片段 | 是 | 不需要 |
