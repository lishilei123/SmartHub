# SmartHub Phase 1 + Phase 2 + Phase 3 完整评审闭环 + Phase 4 前端预览
...
第四期多 Agent 测试设计工作流已完成需求/技术设计和交互式前端预览，后端 DAG、API、持久化和真实 Agent 运行尚未实现。预览覆盖双依据模式、四 Agent 五阶段产物链、测试点树、历史复用、结构化用例、数据需求、覆盖审计、人工发布，以及项目测试用例库和功能/冒烟/回归/功能域用例集。一条用例可同时声明 UI 与 API 执行方式，并分别维护步骤和断言；本次新功能集只包含新增/修改/复用用例，影响回归使用固定引用，冒烟候选与既有基线组合后可生成标准/快速/直接全量执行交接。第四期仍不创建真实测试数据、`TestPlanVersion` 或测试任务，不执行脚本，也不发布新的冒烟基线；这些由后续执行阶段基于真实结果和人工决定完成。
...
- Phase 4 交互式前端预览：测试设计列表/创建/工作台、项目测试用例库、用例集目录、新功能集组成、冒烟候选与组合快照、三种执行交接策略，以及单条用例 UI/API 多选编辑；当前使用预览数据，不代表后端能力已交付；
