...
- [一期技术方案（含架构设计）](技术方案/第一期-项目知识库构建与配置技术方案.md)
...
- SmartHub 本地模型来源始终存在且不可删除，其中的模型可以添加、运行、停止或全部删除；删除运行中模型会先释放实例。“添加远程来源”仅用于接入远程 API。向量维度不需要手工填写，本地模型启动后从运行时读取，远程模型通过一次不指定维度的 Embedding 请求自动检测；远程调用失败时明确失败而不降级为 Hash 向量；
...
- Reranker 可独立选择模型来源和模型；重排阶段按所选来源使用对应的本地运行实例或远程 Base URL/API Key，不要求与知识库 Embedding 模型相同；
...
进入“系统管理 → 知识库配置”后，系统内置的“本地模型”来源始终存在，可以继续添加多个远程 API 来源；每个来源都能配置多个模型。本地模型可分别点击“运行/停止”，页面会展示每个实例的真实状态；最后在“知识库生效模型”中依次选择来源和模型。SmartHub 内置模型由 API 进程直接运行，不需要安装 Ollama，也不需要填写本地地址。远程来源同时兼容 OpenAI Embeddings 和 Ollama 原生协议：OpenAI 兼容服务填写 `http://host:port/v1`，Ollama 可填写 `http://localhost:11434/api` 或完整的 `http://localhost:11434/api/embed`。默认目录如下：
