...
test('MaintenanceProposal 退出执行主流程，历史治理 API 与正式 editor 深链仍保持人工边界', () => {
  ...
  assert.doesNotMatch(taskPanel, /maintenance|用例维护|接受建议|拒绝建议|MaintenanceProposal/u)
  assert.doesNotMatch(runPanel, /maintenance|待确认|MaintenanceProposal/u)
  assert.doesNotMatch(page, /proposal\.taskId === task\.id|loadMaintenanceProposals|decideMaintenanceProposal/u)
  assert.doesNotMatch(hook, /loadMaintenanceProposals|decideMaintenanceProposal|已确认该测试用例需要人工维护/u)
  assert.match(taskPanel, /tab === 'attempts'/u)
  assert.match(taskPanel, /tab === 'diagnoses'/u)
  assert.match(taskPanel, /tab === 'scripts'/u)
  assert.match(taskPanel, /tab === 'artifacts'/u)
  assert.match(api, /headers: \{ 'if-match': etag \}/u)
  assert.match(api, /JSON\.stringify\(\{ decision \}\)/u)

  assert.doesNotMatch(page, /searchParams\.set\('testDesignEntry', 'library'\)/u)
  assert.doesNotMatch(page, /searchParams\.set\('libraryCaseId', caseId\)/u)
