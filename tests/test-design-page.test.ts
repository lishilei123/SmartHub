...
test('真实测试用例支持 ETag 编辑、审核、阻断修复和不可变发布', () => {
  ...
  assert.match(source, /Agent 自动修复/)
  for (const label of ['Agent 质量阻断', '待人工审核', '待人工决策', '需人工修订']) assert.match(source, new RegExp(label))
  assert.match(source, /coverageBlockerResolution/)
