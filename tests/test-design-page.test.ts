...
test('测试执行与报告诊断均使用真实懒加载页面', () => {
  ...
  for (const boundary of ['PostgreSQL', 'Artifact Store', 'Deterministic AgentRunner', 'Agent Under Test', '全部正式用例']) assert.match(`${page}\n${runPanel}`, new RegExp(boundary, 'u'))
  for (const history of ['执行方式', '执行次数', '平均耗时', '成功率', 'Agent Execution Result', 'Deterministic Assertions', 'AI Evaluation', 'Trace / Evidence', '诊断', '脚本历史', '运行产物']) assert.match(taskPanel, new RegExp(history, 'u'))
  for (const internalDetail of ['State version', 'Agent snapshots', '配置 Hash']) assert.doesNotMatch(`${taskPanel}\n${runPanel}`, new RegExp(internalDetail, 'u'))
  for (const executionProcess of ['执行过程', 'ExecutionEventTimeline', 'Query 字段', 'Playwright Reporter', 'te-event-artifacts']) assert.match(taskPanel, new RegExp(executionProcess, 'u'))
  assert.match(taskPanel, /event\.metadata\?\.method/u)
  assert.match(taskPanel, /event\.metadata\?\.path/u)
  assert.match(taskPanel, /event\.metadata\?\.httpStatus/u)
  assert.match(taskPanel, /artifactUrl\(artifact\.id, artifact\.type === 'screenshot' \? 'inline' : 'attachment'\)/u)
  assert.doesNotMatch(taskPanel, /JSON\.stringify\(event\.metadata/u)
  assert.match(api, /JSON\.stringify\(\{ agentUnderTestId \}\)/u)
  for (const contract of ['执行范围', '全部正式用例', '创建 Run', 'Agent Under Test', 'Endpoint', 'type="url"']) assert.match(runPanel, new RegExp(contract, 'u'))
