...
test('历史匹配由 Service 管理且审核页不暴露 Proposal', () => {
  ...
  assert.match(service, /findHistoricalSemanticMatch/u)
...
test('测试用例支持新增、结构化编辑、单条审核与 Revision 记录', () => {
  ...
  const hook = read('../src/test-design/hooks/useTestDesign.ts')
  const service = read('../server/application/test-design-service.ts')
  for (const label of ['新增', '测试用例审核', '批量审核', '提交审核', '退回修改', '拒绝', '前置条件', '执行步骤', '预期结果', '扩展测试']) assert.match(panel, new RegExp(label, 'u'))
...
test('Coverage 面板只展示确定性 v3 审计，发布面板显示正式资产文件', () => {
  ...
  assert.match(audit, /显式 Requirement 引用/u)
