...
test('测试用例支持新增、结构化编辑、单条审核与 Revision 记录', () => {
  ...
  for (const label of ['新增用例', '待人工审核', '批量审核通过', '编辑修改并新建 Revision', '审核通过', 'Revision 与审核记录', '确认删除']) assert.match(panel, new RegExp(label, 'u'))
  ...
  assert.doesNotMatch(panel, />提交审核</u)
  assert.doesNotMatch(panel, />要求修改</u)
  assert.doesNotMatch(panel, />撤回审核</u)
  assert.doesNotMatch(service, /TEST_CASE_EDIT_LOCKED/u)
  assert.match(service, /function newCase[^\n]+reviewState: 'in_review'/u)
  assert.match(service, /draft: \{ submit: 'in_review', approve: 'approved' \}/u)
  assert.match(service, /testCase\.reviewState = 'in_review'/u)
