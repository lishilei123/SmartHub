...
test('历史匹配由 Service 管理且审核页不暴露 Proposal', () => {
  const page = read('../src/test-design/TestDesignPage.tsx')
  const workflow = read('../server/application/test-design/workflow.ts')
  const review = read('../server/application/test-design/case-review.ts')
  assert.doesNotMatch(page, /CaseChangeProposalPanel|用例库变更/u)
  assert.match(workflow, /matchHistoricalCandidate/u)
  assert.match(review, /buildEffectiveCaseSet/u)
  assert.match(workflow, /materializeCaseChangeProposals/u)
  assert.match(review, /reconcileAutomaticProposalDecisions/u)
  assert.match(review, /TEST_DESIGN_SERVICE_ACTOR_ID/u)
})
...
test('测试用例支持新增、结构化编辑、单条审核与 Revision 记录', () => {
  ...
  const review = read('../server/application/test-design/case-review.ts')
  assert.match(review, /TEST_CASE_REVIEW_COMMENT_REQUIRED/u)
  assert.match(review, /function newCase[\s\S]+?reviewState: 'in_review'/u)
  assert.match(review, /draft: \{ submit: 'in_review' \}/u)
...
test('测试执行与报告诊断均使用真实懒加载页面', () => {
  ...
  for (const history of ['执行方式', '执行次数', '总耗时', '自动修复', '诊断', '脚本历史', '运行产物']) assert.match(taskPanel, new RegExp(history, 'u'))
  ...
  for (const executionProcess of ['执行过程', 'ExecutionEventTimeline', 'Query 字段', '已返回', '失败位置', 'Playwright Reporter', 'te-event-artifacts']) assert.match(taskPanel, new RegExp(executionProcess, 'u'))
  ...
  assert.match(readme, /\| 测试执行 \|/u)
  assert.match(readme, /\| 报告与诊断 \|/u)
