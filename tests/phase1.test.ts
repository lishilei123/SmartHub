...
test('知识库目录创建、重命名和删除均持久化', async () => {
  ...
  const synced = await ingestReady(service, { knowledgeBaseId: kbId, sourceType: 'upload', sourceKey: 'browser:需求文档/支付/a.md', assetType: '需求', displayName: 'a.md', logicalPath: '需求文档/支付/a.md', content: document() })
  ...
  const deletion = await reloaded.deleteDirectory(requirements.id, 'recursive')
