...
test('PostgreSQL Phase 4 使用规范化事实表并以 nodeRunId 隔离 fencing token', async () => {
  ...
  const run: TestDesignWorkflowRun = {
    ...
    basisSnapshot: { schemaVersion: 'test-design-basis-snapshot/v3', projectVersionId: ids.projectVersion, requirementReleaseId: `${prefix}-release`, verificationRunId: `${prefix}-verification`, requirementReleaseContentSha256: '2'.repeat(64), content: { requirements: [{ clientRequirementPointId: 'RP-1', title: '固定需求', description: '固定内容', actor: '用户', action: '操作', object: '对象', conditions: [], businessRules: [], exceptions: [], acceptanceCriteria: [], evidenceRefs: [], coverageTarget: true }], evidence: [], clarifications: [], testFocus: [] }, snapshotSha256: '4'.repeat(64), createdAt: now },
    ...
    historicalSnapshot: { schemaVersion: 'historical-case-snapshot/v2', items: [], requirementMappings: [], snapshotSha256: '6'.repeat(64), createdAt: now },
    nodeRuns: [{ id: nodeRunId, nodeKey: 'test_case_design', generation: 1, attempt: 0, status: 'queued', dependencies: [] }], artifacts: [], gateDecisions: [], testCases: [], caseChangeProposals: [], coverageAudits: [], events: [], createdBy: 'integration-test', createdAt: now,
  }
