...
test('PostgreSQL Migration 26 完整恢复追溯、冻结 Revision、readiness override 和幂等迁移', async () => {
  ...
  const run: TestDesignWorkflowRun = {
    ...
    basisSnapshot: { schemaVersion: 'test-design-basis-snapshot/v3', projectVersionId: ids.projectVersion, requirementReleaseId: traceability.sourceRequirementReleaseId, verificationRunId: `${prefix}-verification`, requirementReleaseContentSha256: '2'.repeat(64), content: { requirements: [], evidence: [], clarifications: [], testFocus: [] }, snapshotSha256: '3'.repeat(64), createdAt: now },
