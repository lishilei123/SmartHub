...
test('报告页恢复 reportRunId 深链并支持列表外 Run 直接加载', () => {
  ...
  assert.match(hook, /api\.loadReport\(projectVersionId, runId, requestController\.signal\)/u)
  assert.match(hook, /controller\.current\?\.abort\(\)/u)
  assert.match(hook, /requestId === reportRequest\.current/u)
  assert.match(hook, /background && selectedRun\.current !== runId/u)
})

test('活动 Run 使用递归 setTimeout 轮询并保留当前报告', () => {
  const hook = read('../src/test-report/hooks/useTestReport.ts')
  assert.match(hook, /activeStatus = report\?\.run\.status/u)
  assert.match(hook, /\['queued', 'running'\]\.includes\(activeStatus\)/u)
  assert.match(hook, /window\.setTimeout\(poll, 1800\)/u)
  assert.match(hook, /catch\s*\{[^}]*delay = 3000/u)
  assert.match(hook, /finally\s*\{[^}]*window\.setTimeout\(poll, delay\)/u)
  assert.match(hook, /if \(controller\.current\) return/u)
  assert.match(hook, /await openReport\(activeRunId, true\)/u)
  assert.match(hook, /setRefreshing\(background\)/u)
  assert.match(hook, /if \(!background\) setReport\(previous => previous\?\.run\.id === runId \? previous : null\)/u)
  assert.match(hook, /window\.clearTimeout\(timer\)/u)
  assert.match(hook, /\[openReport, activeRunId, activeStatus, selection\]/u)
