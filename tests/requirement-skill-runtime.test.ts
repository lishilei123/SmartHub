...
test('System Prompt 只包含 Enabled Skill Catalog，不包含任何 Skill 正文', async () => {
  ...
  assert.match(prompt, /- test-case-design[\s\S]*version: 1\.4\.0/u)
...
test('skill.read 返回当前绑定版本的 TRUSTED_SKILL 正文，并在同轮缓存重放', async () => {
  ...
  assert.equal(firstData.version, '1.4.0')
  assert.match(firstData.content, /^<<<TRUSTED_SKILL key="test-case-design" version="1\.4\.0">>>/u)
