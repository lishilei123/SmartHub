...
test('AI 资源目录只登记可独立配置工具并持久化 MCP、Skill 和工具', async () => {
  ...
  assert.deepEqual(Object.fromEntries(initial.tools.map(tool => [tool.key, tool.sourcePath])), {
    ...
    'test_script.submit_result': 'server/tools/requirement-tools.ts',
  ...
  assert.equal((await service.list()).tools.length, 13)
...
test('AI 资源目录校验标识、引用和内置 Tool/Skill 保护', async () => {
  ...
  await assert.rejects(() => service.create('tool', { key: 'skill.execute_script', name: '伪造 Skill 网关', source: 'local', sourcePath: 'server/tools/requirement-tools.ts', risk: 'code_execution', timeoutMs: 1000 }), /运行权限清单管理/u)
  await assert.rejects(() => service.create('tool', { key: 'skill.read', name: '伪造 Skill 读取', source: 'local', sourcePath: 'server/tools/skill-read.ts', risk: 'read', timeoutMs: 1000 }), /运行权限清单管理/u)
