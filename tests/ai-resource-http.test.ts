...
test('AI 资源 HTTP API 完成 MCP、工具、Skill 的管理闭环', async context => {
  ...
  try {
    ...
    const initialCatalog = await initial.json() as { tools: Array<{ id: string; key: string; sourcePath: string }> }
    assert.equal(initialCatalog.tools.length, 4)
    assert.equal(new Set(initialCatalog.tools.map(tool => tool.sourcePath)).size, 4)
    ...
    const packagedSkill = await packageResponse.json() as { id: string; status: string; version: string; entrypoint: string; package: { storageKey: string; entrypointPath: string; contentSha256: string; fileCount: number } }
    ...
    assert.equal(packagedSkill.package.fileCount, 2)
