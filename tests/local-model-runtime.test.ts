...
test('本地模式的上传解析和混合检索使用系统内置模型向量', async () => {
  ...
  assert.deepEqual(synced.version.chunks[0].embedding, [17, 1, 1])
...
test('旧版 Hash 模型配置在启动时迁移为系统内置模型', async () => {
  ...
  await service.initialize()
  assert.equal(service.config(kbId).config.embeddingModel, 'Xenova/paraphrase-multilingual-MiniLM-L12-v2')
  assert.equal(service.config(kbId).config.embeddingDimensions, 384)
})
